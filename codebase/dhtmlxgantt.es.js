...
dhtmlxGantt v.9.1.2 Professional
...
function Xe(t) {
  ...
  t._isLinksCacheEnabled = function() {
    ...
    }), k = 0, f = m.length; k < f; k++) for (var p = m[k], y = 0, $ = b.length; y < $; y++) {
    ...
    for (var s, r = n === void 0 ? t.config.root_id : n, o = (e = {}, {}), l = [{ from: r, includePredecessors: i, isChild: !1 }]; l.length; ) {
      ...
      if (!e[r = d.from]) {
        ...
        for (var c = this._collectRelations(s, u, i, o), h = 0; h < c.length; h++) {
          ...
          const b = t.getTask(_.target), k = t.getTask(_.source);
          (b.unscheduled || k.unscheduled) && (m = !1), m && (a[_.hashSum] = _);
...
class je {
  ...
  resolveRelationDate(n, e, i) {
    ...
    for (let _ = 0; _ < d.length; _++) {
      const g = d[_];
      o = g.preferredStart;
      const v = this.getLatestEndDate(g, i, l), m = this._gantt.calculateEndDate({ start_date: v, duration: -l.duration, task: l });
      this._comparator.isGreaterOrDefault(u, v, l) && (u = v), this._comparator.isGreaterOrDefault(o, m, l) && this._comparator.isGreaterOrDefault(a, v, l) && (a = v, r = m, s = g.id);
    }
  ...
  resolveRelationDate(n, e, i) {
    ...
    for (let v = 0; v < l.length; v++) {
      ...
      if (this._comparator.isSmallerOrDefault(c, b, o) && (c = b), this._comparator.isSmallerOrDefault(r, b, o) && this._comparator.isSmallerOrDefault(a, b, o) && (a = b, s = m.id), !o.duration) {
        const k = this._gantt.getLink(m.id);
        (u[k.type] === void 0 || u[k.type] < +b) && (u[k.type] = +b);
      }
    }
    ...
    for (const o in s) {
      ...
      c && (d = { start_date: c.start_date, end_date: c.end_date }), h && (d = { start_date: h.start_date, end_date: h.end_date }), n._formatLink(l, d, u).forEach(function(_) {
        for (const g in e) e[g].predecessors.forEach(function(v) {
          const m = v.id === _.id, b = v.target === _.target, k = v.source === _.source;
          m && b && k && (v.lag = _.lag, v.sourceLag = _.sourceLag, v.targetLag = _.targetLag, v.hashSum = _.hashSum);
        });
      }), r = !0;
    }
    return r;
...
function hi(t, n, e, i) {
  const a = function() {
    ...
    t.attachEvent("onAfterBatchUpdate", function() {
      ...
      return t._getAutoSchedulingConfig().enabled && (t.getState().drag_mode !== "progress" && (s = _(x)), r = x), !0;
    });
    ...
    t.attachEvent("onBeforeTaskChanged", function(x, w, T) {
      return k(x, T), y[x] = T, !0;
    }), t.attachEvent("onAfterTaskDrag", function(x, w, T) {
      x === r && (clearTimeout($), $ = setTimeout(function() {
        (function(S, C) {
      ...
      if (t._getAutoSchedulingConfig().enabled && !t._autoscheduling_in_progress) {
        f = !1;
        const T = t.getTask(x);
        h(w, T) && (p = x, v(w, T) && (w.$keep_constraints = !0), t.getConstraintType(w) === t.getConstraintType(T) && +w.constraint_date == +T.constraint_date || (f = !0));
      }
      return !0;
    }), t.attachEvent("onAfterTaskUpdate", function(x, w) {
      return t._getAutoSchedulingConfig().enabled && !t._autoscheduling_in_progress && p !== null && p == x && (p = null, w.$keep_constraints ? delete w.$keep_constraints : f || m(w), t.autoSchedule(w.id), f || b(w)), !0;
    });
  };
...
const Pn = Object.freeze(Object.defineProperty({ __proto__: null, addClassName: xt, closest: ct, getActiveElement: De, getChildNodes: In, getClassName: it, getClosestSizedElement: Ln, getFocusableNodes: Vt, getNodePosition: Y, getRelativeEventPosition: dt, getRelativeNodePosition: Mn, getRootNode: Et, getScrollSize: En, getTargetNode: At, hasClass: function(t, n) {
  ...
  attach(t, n, e) {
    ...
    this._domEvents.attach(l, "mouseup", (g) => {
      ...
      if (this._touchActive) {
        let v = u();
        if (v && i.utils.dom.closest(g.target, v)) return;
    ...
    const u = () => {
      ...
      r = null;
      let v = u();
      v && i.utils.dom.closest(g.target, v) || (s.registerProvider("clickDrag", () => ({ autoscroll: this._mouseDown })), n && g[n] !== !0 || (r = this._getCoordinates(g, t)));
    }, h = (g) => {
      ...
      if (n && g[n] !== !0) return;
      const v = this._gantt.ext.clickDrag, m = (this._gantt.config.drag_timeline || {}).useKey;
      if (v && m && !n && g[m]) return;
      let b = null;
...
class bi {
  ...
  _show_quick_info(n, e) {
    ...
    if (i.config.quick_info_detached) {
      ...
      const g = Math.min(Math.max(l.x, n.left - n.dx * (r - n.width)), c), v = _;
...
const Ti = { auto_scheduling: function(t) {
      ...
      if (_.length) t.callEvent("onAutoScheduleCircularLink", [_]);
        ...
        (function(v, m) {
          if (!t._getAutoSchedulingConfig().apply_constraints) for (var b = 0; b < m.length; b++) {
            var k = m[b], f = t.getTask(k.target);
            t._getAutoSchedulingConfig().gap_behavior !== "preserve" && k.target != v || (k.preferredStart = new Date(f.start_date));
          }
        })(d, u);
        ...
        (function(v) {
          v.length && t.batchUpdate(function() {
            for (var m = 0; m < v.length; m++) t.updateTask(v[m]);
          }, l);
  ...
  var n = function(i) {
    var a = en(i), s = tn(), r = { _freeSlack: {}, _totalSlack: {}, _slackNeedCalculate: !0, _linkedTasksById: {}, _successorsByTaskId: {}, _projectEnd: null, _calculateSlacks: function() {
      ...
      const _ = { _cache: {}, getDist: function(g, v) {
        ...
        {
          const b = i.calculateDuration({ start_date: g.end_date, end_date: v.start_date, task: g });
      ...
      o.forEach((g) => {
        ...
        if (!v) return;
        _[g.source] = !0;
        let m = u.getDist(v, i.getTask(g.target));
      ...
      for (var c = {}, h = i.getTaskByTime(), _ = 0; _ < l.length; _++) {
        ...
        if (this._isCompletedTask(v)) c[v.id] = 0;
          ...
          const m = d[v.id].linked;
          let b = null;
          for (var g = 0; g < m.length; g++) {
            const k = m[g], f = i.getTask(k.target);
            let p = 0;
            c[f.id] !== void 0 && (p += c[f.id]), p += Math.max(u.getDist(v, f) - k.link.targetLag, 0), p -= k.link.trueLag || 0, b = b === null ? p : Math.min(b, p);
          }
          c[v.id] = b || 0;
  ...
  (function(n) {
    n.config.keyboard_navigation = !0, n.config.keyboard_navigation_cells = !1, n.$keyboardNavigation = {}, n._compose = function() {
      ...
      var i = function(m) {
        ...
        if (b && !m) {
          var k, f, p = b.getNode();
          p && p.parentNode && (k = p.parentNode.scrollTop, f = p.parentNode.scrollLeft), b.focus(!0), p && p.parentNode && (p.parentNode.scrollTop = k, p.parentNode.scrollLeft = f);
        }
      };
      function r(m) {
      ...
      function l(m) {
        ...
        if (!o(m)) {
          var b, k = e.fromDomElement(m);
          if (k && (e.activeNode instanceof n.$keyboardNavigation.TaskCell && Z(m.target, n.$task) && (k = new n.$keyboardNavigation.TaskCell(k.taskId, e.activeNode.columnIndex)), b = k, n.config.show_grid && n.$ui.getView("grid") && n.config.keyboard_navigation_cells)) {
            const f = m.target.classList.contains("gantt_row"), p = m.target.closest(".gantt_task_line"), y = n.utils.dom.getNodePosition(n.$grid).x, $ = y + n.$grid.offsetWidth, x = n.utils.dom.getNodePosition(document.activeElement).x;
            if (f || p && (x < y || $ < x)) {
      ...
      var d = n.attachEvent("onGanttReady", function() {
        if (n.detachEvent(d), n.$data.tasksStore.attachEvent("onStoreUpdated", function(b) {
          if (n.config.keyboard_navigation && e.isEnabled()) {
            const k = e.getActiveNode(), f = n.$ui.getView("grid");
            if (!f || !f.$grid_data) return;
            const p = f.getItemTop(b), y = f.$grid_data.scrollTop, $ = y + f.$grid_data.getBoundingClientRect().height;
            k && k.taskId == b && y <= p && $ >= p && s();
          }
        }), n._smart_render) {
          var m = n._smart_render._redrawTasks;
          n._smart_render._redrawTasks = function(b, k) {
            if (n.config.keyboard_navigation && e.isEnabled()) {
              var f = e.getActiveNode();
              if (f && f.taskId !== void 0) {
                for (var p = !1, y = 0; y < k.length; y++) if (k[y].id == f.taskId && k[y].start_date) {
                  p = !0;
                  break;
                }
                p || k.push(n.getTask(f.taskId));
              }
            }
      ...
      n.attachEvent("onTaskCreated", function(m) {
        ...
        if (e.isEnabled()) {
          ...
          var k = 0, f = e.activeNode;
          f instanceof n.$keyboardNavigation.TaskCell && (k = f.columnIndex);
          var p = n.config.keyboard_navigation_cells ? n.$keyboardNavigation.TaskCell : n.$keyboardNavigation.TaskRow;
          b.type == n.config.types.placeholder && n.config.placeholder_task.focusOnCreate === !1 || e.setActiveNode(new p(m, k));
        }
      }), n.attachEvent("onTaskIdChange", function(m, b) {
        if (!n.config.keyboard_navigation) return !0;
        var k = e.activeNode;
        return e.isTaskFocused(m) && (k.taskId = b), !0;
      });
      ...
      function g(m) {
        for (var b = n.getGridColumns(), k = 0; k < b.length; k++) if (b[k].name == m) return k;
        return 0;
      }
      ...
      var v = {};
      _t(v), n.mixin(v, { addShortcut: function(m, b, k) {
        var f = _(k);
        f && f.prototype.bind(m, b);
      }, getShortcutHandler: function(m, b) {
        var k = n.$keyboardNavigation.shortcuts.parse(m);
        if (k.length) return v.getCommandHandler(k[0], b);
      }, getCommandHandler: function(m, b) {
        var k = _(b);
        if (k && m) return k.prototype.findHandler(m);
      }, removeShortcut: function(m, b) {
        var k = _(b);
        k && k.prototype.unbind(m);
      }, focus: function(m) {
        var b, k = m ? m.type : null, f = _(k);
        switch (k) {
          case "taskCell":
        ...
        if (e.isEnabled()) {
          var m = e.getActiveNode(), b = (f = m) instanceof n.$keyboardNavigation.GanttNode ? "gantt" : f instanceof n.$keyboardNavigation.HeaderCell ? "headerCell" : f instanceof n.$keyboardNavigation.TaskRow ? "taskRow" : f instanceof n.$keyboardNavigation.TaskCell ? "taskCell" : null, k = n.getGridColumns();
          switch (b) {
            case "taskCell":
              return { type: "taskCell", id: m.taskId, column: k[m.columnIndex].name };
            case "taskRow":
              return { type: "taskRow", id: m.taskId };
            case "headerCell":
              return { type: "headerCell", column: k[m.index].name };
          }
        }
        var f;
        return null;
  ...
  function l(u, c, h) {
    ...
    for (let g = 0; g < u.length; g++) {
      const v = u[g];
      for (let m = 0; m < v.commands.length; m++) v.commands[m].entity === _.command.entity.task ? s(v.commands[m], c, h) : v.commands[m].entity === _.command.entity.link && o(v.commands[m], c, h);
    }
  }
  function d(u, c, h) {
    const _ = n;
    for (let g = 0; g < u.length; g++) {
      const v = u[g];
      for (let m = 0; m < v.commands.length; m++) {
        const b = v.commands[m];
  ...
  function n(l, d, u) {
    ...
    for (let h = 0; h < l.length; h++) {
      ...
      }).reduce(function(v, m, b) {
        return m === _ && v.push(b), v;
      }, []);
  ...
  t._groups = { relation_property: null, relation_id_property: "$group_id", group_id: null, group_text: null, loading: !1, loaded: 0, dynamicGroups: !1, set_relation_value: void 0, _searchCache: null, init: function(l) {
    ...
    this._searchCache = null, l.attachEvent("onBeforeTaskMove", function(h, _, g) {
      var v = _ === this.config.root_id, m = this._groups.dynamicGroups && !(this._groups.set_relation_value instanceof Function);
      if (d.is_active() && (v || m)) return !1;
      var b = l.getTask(h);
      if (this._groups.save_tree_structure && l.isTaskExists(b.parent) && l.isTaskExists(_)) {
        var k = l.getTask(b.parent), f = l.getTask(_);
        f.$virtual && l.isChildOf(k.id, f.id) && (b.parent = l.config.root_id);
        let p = !1, y = f;
      ...
      if (l.config.order_branch && l.config.order_branch != "marker") {
        ...
        if (g.$initial_parent) {
          if (g.parent == l.config.root_id) {
            var v = l.getTask(g.$rendered_parent), m = l.getTask(g.$initial_parent), b = !1;
            this._groups.dynamicGroups && v[this._groups.group_id] != m[this._groups.group_id] && (b = !0), this._groups.dynamicGroups || v[this._groups.group_id] == m[this._groups.relation_property] || (b = !0), b && (g.parent = g.$initial_parent);
          }
    ...
    const v = l._getAutoSchedulingConfig().schedule_on_parse, m = l.config.auto_scheduling_initial;
    m ? l.config.auto_scheduling_initial = !1 : v && (l.config.auto_scheduling.schedule_on_parse = !1), l.parse(_), g.forEach(function(b) {
      l.isTaskExists(b) && l.selectTask(b);
    }), m ? l.config.auto_scheduling_initial = v : v && (l.config.auto_scheduling.schedule_on_parse = v);
    ...
    this._groups.regroup = function() {
      var v = t.getTaskByTime(), m = {}, b = !1;
      v.forEach(function(f) {
        f.$virtual && f.$open !== void 0 && (m[f[_]] = f.$open, b = !0);
      });
      var k = function(f, p, y) {
        var $;
        return $ = f.groups ? y._groups.dynamicGroups ? function(x, w) {
      ...
      return k && b && k.forEach(function(f) {
        m[f[_]] !== void 0 && (f.open = m[f[_]]);
      }), d._groups.group_tasks(d, k, h, _, g), !0;
...
function Pi(t) {
  var n = function(u) {
    ...
    if (u.config.scale_offset_minimal) {
      const g = new Ve(u), v = [g.primaryScale()].concat(g.getAdditionalScales());
  ...
  _sendData(t, n) {
    ...
    if (this._tMode === "CUSTOM") {
      ...
      const u = this.getGanttMode(), c = (_) => {
        let g = l || "updated", v = n, m = n;
        _ && (g = _.action || l, v = _.sid || v, m = _.id || _.tid || m), this.afterUpdateCallback(v, m, g, _, u);
      };
      let h;
  ...
  function r(l, d, u) {
    ...
    return ht(d, function(g) {
      ...
      l in g && ht(Lt(g[l]) ? g[l] : [g[l]], function(v) {
        var m = v && v.resource_id ? v.resource_id : v;
        if (_[a(m)]) c.push(g);
        else if (!e) {
          var b = s(v, l);
          n[b] || (n[b] = []), n[b].push(g);
          ...
          if (u) {
            ...
            if (b.start_date.valueOf() === _.valueOf() && b.end_date.valueOf() === g.valueOf()) b.value = l, l ? v.updateItem(b.id) : v.removeItem(b.id);
              ...
              if (b.end_date.valueOf() > g.valueOf()) {
                const k = t.copy(b);
                k.id = t.uid(), k.start_date = g, k.duration = t.calculateDuration({ start_date: k.start_date, end_date: k.end_date, task: m }), k.delay = t.calculateDuration({ start_date: m.start_date, end_date: k.start_date, task: m }), k.mode = b.mode || "default", k.duration !== 0 && v.addItem(k);
              }
              b.start_date.valueOf() < _.valueOf() ? (b.end_date = _, b.duration = t.calculateDuration({ start_date: b.start_date, end_date: b.end_date, task: m }), b.mode = "fixedDuration", b.duration === 0 ? v.removeItem(b.id) : v.updateItem(b.id)) : v.removeItem(b.id), l && v.addItem({ task_id: b.task_id, resource_id: b.resource_id, value: l, start_date: _, end_date: g, duration: t.calculateDuration({ start_date: _, end_date: g, task: m }), delay: t.calculateDuration({ start_date: m.start_date, end_date: _, task: m }), mode: "fixedDuration" });
            }
            ...
            v.addItem(b), t.updateTaskAssignments(m.id), t.updateTask(m.id);
          }
        }
      }
    }
...
function Ui(t) {
    ...
    if (t.defined($) && $) {
      ...
      $.forEach(function(S) {
        ...
        Object.keys(S).forEach((I) => {
          I != "$id" && (A[I] = S[I]);
        }), A.start_date && A.start_date.getMonth && A.end_date && A.end_date.getMonth && typeof A.duration == "number" || c(A, p), x.push(A);
      });
    }
    return x;
  }
  ...
  function m(p, y) {
    ...
    }), o == r ? p[t.config.resource_property] = y : o == s && (p[t.config.resource_property] = y.map(function($) {
  ...
  function k(p, y) {
    var $ = function(x, w) {
      ...
      } else o != r && o != s || (A = {}, I = {}, M = {}, x.forEach(function(D) {
        A[D.id || D.$id] = D;
      }), w.forEach(function(D) {
        I[D.id] = D;
      }), x.concat(w).forEach(function(D) {
    ...
    if (t.config.process_resource_assignments) {
      ...
      t.attachEvent("onBeforeBatchUpdate", function() {
        ...
        if (y) {
          ...
          for (var E in t.config.process_resource_assignments && o === "resourceValueArray" && (S = null), $) k($[E], C[E]);
        }
        y = !1, p = !1, $ = {};
...
const Ge = function(t, n) {
  ...
  if (o.splitChild && (h = Math.floor((l - d) / 2)), s || r) {
    let _ = Math.min(e.height, l) - d, g = 2, v = r && a.bar_height >= a.row_height, m = o.splitChild && e.height >= l;
    (v || m) && (g = 0), h = Math.floor(_ / 2) + g, e.rowHeight;
  }
...
function Ki(t) {
  ...
  function a(s) {
    ...
    if (t.isTaskExists(r)) {
      ...
      l && o.baselines.push(s), q(t) || (pt(o) ? e(o) : t.adjustTaskHeightForBaselines(o));
    }
  }
...
Gt.prototype = { init: function(t) {
  ...
  var l = function(u, c) {
    var h = { column_before_start: u.bind(function(_, g, v) {
      var m = c.$getConfig(), b = et(v, m.grid_resizer_column_attribute);
      if (!b || !ct(b, ".gantt_grid_column_resize_wrap")) return !1;
      var k = this.locate(v, m.grid_resizer_column_attribute), f = c.getGridColumns()[k];
      return c.callEvent("onColumnResizeStart", [k, f]) !== !1 && void 0;
    }, u), column_after_start: u.bind(function(_, g, v) {
      var m = c.$getConfig(), b = this.locate(v, m.grid_resizer_column_attribute);
      ...
      var m = c.$getConfig(), b = _.config, k = c.getGridColumns(), f = parseInt(b.drag_index, 10), p = k[f], y = Y(c.$grid_scale), $ = parseInt(b.marker.style.left, 10), x = p.min_width ? p.min_width : m.min_grid_column_width, w = c.$grid_data.offsetWidth, T = 0, S = 0;
      m.rtl ? $ = y.x + y.width - 1 - $ : $ -= y.x - 1;
      for (var C = 0; C < f; C++) x += k[C].width, T += k[C].width;
      if ($ < x && ($ = x), m.keep_grid_width) {
        var E = 0;
        for (C = f + 1; C < k.length; C++) k[C].min_width ? w -= k[C].min_width : m.min_grid_column_width && (w -= m.min_grid_column_width), k[C].max_width && E !== !1 ? E += k[C].max_width : E = !1;
        E && (x = c.$grid_data.offsetWidth - E), $ < x && ($ = x), $ > w && ($ = w);
      } else if (!c.$config.scrollable) {
        var A = $, I = u.$container.offsetWidth, M = 0;
        if (c.$grid_data.offsetWidth <= I - 25) for (C = f + 1; C < k.length; C++) M += k[C].width;
        else {
          for (C = f + 1; C >= 0; C--) M += k[C].width;
          M = I - M;
        }
        M > I && (M -= I);
      ...
      return b.left = $ - 1, S = Math.abs($ - T), p.max_width && S > p.max_width && (S = p.max_width), m.rtl && (T = y.width - T + 2 - S), b.marker.style.top = y.y + "px", b.marker.style.left = y.x - 1 + T + "px", b.marker.style.width = S + "px", c.callEvent("onColumnResize", [f, k[f], S - 1]), !0;
    }, u), column_drag_end: u.bind(function(_, g, v) {
      for (var m = c.$getConfig(), b = c.getGridColumns(), k = 0, f = parseInt(_.config.drag_index, 10), p = b[f], y = 0; y < f; y++) k += b[y].width;
      var $ = p.min_width && _.config.left - k < p.min_width ? p.min_width : _.config.left - k;
      if (p.max_width && p.max_width < $ && ($ = p.max_width), c.callEvent("onColumnResizeEnd", [f, p, $]) !== !1 && p.width != $) {
        if (p.width = $, m.keep_grid_width) k = m.grid_width;
        else {
          y = f;
          for (var x = b.length; y < x; y++) k += b[y].width;
        }
        c.callEvent("onColumnResizeComplete", [b, c._setColumnsWidth(k, f)]), c.$config.scrollable || u.$layout._syncCellSizes(c.$config.group, { value: m.grid_width, isGravity: !1 }), this.render();
      }
    }, u) };
    return { init: function() {
      var _ = u.$services.getService("dnd"), g = c.$getConfig(), v = new _(c.$grid_scale, { updates_per_second: 60 });
      u.defined(g.dnd_sensitivity) && (v.config.sensitivity = g.dnd_sensitivity), v.attachEvent("onBeforeDragStart", function(m, b) {
        return h.column_before_start(v, m, b);
      }), v.attachEvent("onAfterDragStart", function(m, b) {
        return h.column_after_start(v, m, b);
      }), v.attachEvent("onDragMove", function(m, b) {
        return h.column_drag_move(v, m, b);
      }), v.attachEvent("onDragEnd", function(m, b) {
        return h.column_drag_end(v, m, b);
      });
    }, doOnRender: function() {
      for (var _ = c.getGridColumns(), g = c.$getConfig(), v = 0, m = c.$config.width, b = g.scale_height, k = 0; k < _.length; k++) {
        var f, p = _[k];
        if (v += p.width, f = g.rtl ? m - v : v, p.resize && k != _.length - 1) {
          var y = document.createElement("div");
          y.className = "gantt_grid_column_resize_wrap", y.style.top = "0px", y.style.height = b + "px", y.innerHTML = "<div class='gantt_grid_column_resize'></div>", y.setAttribute(g.grid_resizer_column_attribute, k), y.setAttribute("column_index", k), u._waiAria.gridSeparatorAttr(y), c.$grid_scale.appendChild(y), y.style.left = Math.max(0, f) + "px";
        }
      }
    } };
  }(n, this);
  ...
  var d = function(u, c) {
    var h = { row_before_start: u.bind(function(_, g, v) {
      ...
      if (!et(v, m.task_grid_row_resizer_attribute)) return !1;
      var k = this.locate(v, m.task_grid_row_resizer_attribute), f = b.getItem(k);
      return c.callEvent("onBeforeRowResize", [f]) !== !1 && void 0;
    }, u), row_after_start: u.bind(function(_, g, v) {
      var m = c.$getConfig(), b = this.locate(v, m.task_grid_row_resizer_attribute);
      ...
      var m = c.$config.rowStore, b = c.$getConfig(), k = _.config, f = k.drag_id, p = c.getItemHeight(f), y = c.getItemTop(f) - g.scrollTop, $ = Y(c.$grid_data), x = parseInt(k.marker.style.top, 10), w = y + $.y, T = 0, S = b.min_task_grid_row_height;
      return (T = x - w) < S && (T = S), k.marker.style.left = $.x + "px", k.marker.style.top = w - 1 + "px", k.marker.style.height = Math.abs(T) + 1 + "px", k.marker_height = T, c.callEvent("onRowResize", [f, m.getItem(f), T + p]), !0;
    }, u), row_drag_end: u.bind(function(_, g, v) {
      var m = c.$config.rowStore, b = _.config, k = b.drag_id, f = m.getItem(k), p = c.getItemHeight(k), y = b.marker_height;
      c.callEvent("onBeforeRowResizeEnd", [k, f, y]) !== !1 && f.row_height != y && (f.row_height = y, m.updateItem(k), c.callEvent("onAfterRowResize", [k, f, p, y]), this.render());
    }, u) };
    return { init: function() {
      var _ = u.$services.getService("dnd"), g = c.$getConfig(), v = new _(c.$grid_data, { updates_per_second: 60 });
      u.defined(g.dnd_sensitivity) && (v.config.sensitivity = g.dnd_sensitivity), v.attachEvent("onBeforeDragStart", function(m, b) {
        return h.row_before_start(v, m, b);
      }), v.attachEvent("onAfterDragStart", function(m, b) {
        return h.row_after_start(v, m, b);
      }), v.attachEvent("onDragMove", function(m, b) {
        return h.row_drag_move(v, m, b);
      }), v.attachEvent("onDragEnd", function(m, b) {
        return h.row_drag_end(v, m, b);
      });
    } };
  }(n, this);
  ...
  for (var u = 0; u < a.length; u++) {
    ...
    var v = t._sort && h.name == t._sort.name ? `<div data-column-id="${h.name}" class="gantt_sort gantt_${t._sort.direction}"></div>` : "", m = ["gantt_grid_head_cell", "gantt_grid_head_" + h.name, c ? "gantt_last_cell" : "", i.grid_header_class(h.name, h)].join(" "), b = "width:" + (_ - (c ? 1 : 0)) + "px;", k = h.label || o["column_" + h.name] || o[h.name];
    typeof k == "function" && (k = k.call(t, h.name, h)), k = k || "";
    let p = !1;
    t.config.external_render && t.config.external_render.isElement(k) && (p = !0, d[h.name] = k);
    var f = "<div class='" + m + "' style='" + b + "' " + t._waiAria.gridScaleCellAttrString(h, k) + " data-column-id='" + h.name + "' column_id='" + h.name + "' data-column-name='" + h.name + "' data-column-index='" + u + "'>" + (p ? "<div data-component-container></div>" : k) + v + "</div>";
    s.push(f);
  }
...
Re.prototype = { units: ["year", "month", "week", "day", "hour", "minute"], _clearCaches: function() {
  ...
  for (var o = 0, l = [], d = 0; a < n; ) {
    ...
    if (l.length && d) if (l[l.length - 1].end <= g && _ > d) a += d, i = this.$gantt.date.add(i, -1, "day");
      ...
      for (var v = !1, m = null, b = null, k = l.length - 1; k >= 0; k--) if (l[k].start < g - 1 && l[k].end >= g - 1) {
        v = !0, m = l[k], b = l[k - 1];
        break;
      }
      if (v) if (g === m.end && _ >= m.durationMinutes) a += m.durationMinutes, i = this.$gantt.date.add(i, -m.durationMinutes, "minute");
  ...
  if (a < n) {
    var x = n - a;
    r = this._subtractMinutesUntilHourStart(i, x), a += r.added, i = r.end;
  }
  return i;
  ...
  t._init_task_timing = function(r) {
    ...
    if (r) {
      ...
      if (l || d.start_date && d.end_date && (c.$no_start || c.$no_end)) {
        const v = d.$auto_start_date ? "$auto_start_date" : "start_date", m = d.$auto_end_date ? "$auto_end_date" : "end_date";
        var _ = d[v].valueOf(), g = d[m].valueOf();
        t.resetProjectDates(d), l || _ != d[v].valueOf() || g != d[m].valueOf() || (h = !1), h && !o && this.refreshTask(d.id, !0), c.scheduled_summary && (h = !0);
      }
...
function Ua() {
  this.constants = Ci, this.version = "9.1.2", this.license = "individual", this.templates = {}, this.ext = {}, this.keys = { edit_save: this.constants.KEY_CODES.ENTER, edit_cancel: this.constants.KEY_CODES.ESC };
}
function Ga(t) {
  ...
  n.plugins = function(l) {
    ...
    return { cache: !0, method: "get", parse: function(d) {
      ...
      return new l.Promise(function(v, m) {
        var b = typeof XMLHttpRequest !== void 0 ? new XMLHttpRequest() : new tt.ActiveXObject("Microsoft.XMLHTTP"), k = navigator.userAgent.match(/AppleWebKit/) !== null && navigator.userAgent.match(/Qt/) !== null && navigator.userAgent.match(/Safari/) !== null;
        h && (b.onreadystatechange = function() {
          if (b.readyState == 4 || k && b.readyState == 3) {
  ...
  var a = function(l) {
    ...
    function u(c, h) {
      ...
      for (var g = { passive: !1 }, v = 0; v < _.length; v++) j(function(m) {
        l.event(c, m.down, j(function(k) {
          if (m.accessor(k) && !(k.button !== void 0 && k.button !== 0 || (h.preventDefault && h.selector && ct(k.target, h.selector) && k.preventDefault(), l.config.touch && k.timeStamp && k.timeStamp - 0 < 300))) if (this._settings.original_target = d(k), this._settings.original_element_sizes = { ...dt(k, Ln(c)), width: k.target.offsetWidth, height: k.target.offsetHeight }, l.config.touch) {
            let f = k.target;
            this.clearDragTimer(), this._drag_start_timer = setTimeout(j(function() {
              l.getState().lightbox || this.dragStart(c, k, m, f);
            }, this), l.config.touch_drag);
          } else this.dragStart(c, k, m);
        }, this), g);
        var b = document.body;
        l.event(b, m.up, j(function(k) {
          m.accessor(k) && this.clearDragTimer();
        }, this), g);
      }, this)(_[v]);
    }
    return u.prototype = { traceDragEvents: function(c, h) {
      ...
      }, this), v = Et(l.$root), m = this.config.mousemoveContainer || Et(l.$root), b = { passive: !1 }, k = j(function(f) {
        return l.eventRemove(m, h.move, g), l.eventRemove(v, h.up, k, b), this.dragEnd(c);
      }, this);
      l.event(m, h.move, g, b), l.event(v, h.up, k, b);
      ...
      v.shadowRoot && _ && (v = _);
      var m = v.cloneNode(!0);
      this.config.original_target = d(g), this.config.original_target.target = m, this.config.backup_element = v, v.parentNode.appendChild(m), v.style.display = "none", (this.config.mousemoveContainer || l.$root || document.body).appendChild(v);
      ...
      if (!this.config.marker && !this.config.started) {
        var m = this.getPosition(v);
        if (l.config.touch || this.checkPositionChange(m)) {
      ...
      if (!this.config.ignore) {
        ...
        if (b instanceof ShadowRoot) {
          if (g && Object.getOwnPropertyDescriptor(v, "target")) v.target = g;
            ...
            const k = h.targetTouches[0];
            v.target = b.elementFromPoint(k.clientX, k.clientY);
          }
        }
  ...
  var s = /* @__PURE__ */ function(l) {
    ...
    return { initTemplates: function() {
      var c = l.date, h = c.date_to_str, _ = l.config, g = h(_.xml_date || _.date_format, _.server_utc), v = c.str_to_date(_.xml_date || _.date_format, _.server_utc);
      u("date_scale", !0, void 0, l.config, l.templates), u("date_grid", !0, "grid_date_format", l.config, l.templates), u("task_date", !0, void 0, l.config, l.templates), l.mixin(l.templates, { xml_format: void 0, format_date: g, xml_date: void 0, parse_date: v, progress_text: function(m, b, k) {
        return "";
      }, grid_header_class: function(m, b) {
        return "";
      }, task_text: function(m, b, k) {
        return k.text;
      }, task_class: function(m, b, k) {
        return "";
      }, task_end_date: function(m) {
        return l.templates.task_date(m);
      }, grid_row_class: function(m, b, k) {
        return "";
      }, task_row_class: function(m, b, k) {
        return "";
      ...
      }, date_grid: function(m, b, k) {
        return b && l.isUnscheduledTask(b) && l.config.show_unscheduled ? l.templates.task_unscheduled_time(b) : l.templates.grid_date_format(m, k);
      }, task_time: function(m, b, k) {
        return l.isUnscheduledTask(k) && l.config.show_unscheduled ? l.templates.task_unscheduled_time(k) : l.templates.task_date(m) + " - " + l.templates.task_end_date(b);
      }, task_unscheduled_time: function(m) {
        return "";
      }, time_picker: h(_.time_picker), link_class: function(m) {
        return "";
      }, link_description: function(m) {
        var b = l.getTask(m.source), k = l.getTask(m.target);
        return "<b>" + b.text + "</b> &ndash;  <b>" + k.text + "</b>";
      }, drag_link: function(m, b, k, f) {
        m = l.getTask(m);
        var p = l.locale.labels, y = "<b>" + m.text + "</b> " + (b ? p.link_start : p.link_end) + "<br/>";
        return k && (y += "<b> " + (k = l.getTask(k)).text + "</b> " + (f ? p.link_start : p.link_end) + "<br/>"), y;
      }, drag_link_class: function(m, b, k, f) {
        var p = "";
        return m && k && (p = " " + (l.isLinkAllowed(m, k, b, f) ? "gantt_link_allow" : "gantt_link_deny")), "gantt_link_tooltip" + p;
      }, tooltip_date_format: c.date_to_str("%Y-%m-%d"), tooltip_text: function(m, b, k) {
        return `<div>Task: ${k.text}</div>
				<div>Start date: ${l.templates.tooltip_date_format(m)}</div>
				<div>End date: ${l.templates.tooltip_date_format(b)}</div>`;
      }, baseline_text: function(m, b, k) {
        return "";
      } });
    }, initTemplate: u };
  }(n);
  ...
  r.registerProvider("global", function() {
    ...
    }, initItem: j(function(k) {
      this.defined(k.id) || (k.id = this.uid()), k.start_date && (k.start_date = l.date.parseDate(k.start_date, "parse_date")), k.end_date && (k.end_date = l.date.parseDate(k.end_date, "parse_date"));
      var f = null;
      (k.duration || k.duration === 0) && (k.duration = f = 1 * k.duration), f && (k.start_date && !k.end_date ? k.end_date = this.calculateEndDate(k) : !k.start_date && k.end_date && (k.start_date = this.calculateEndDate({ start_date: k.end_date, duration: -k.duration, task: k }))), l.config.deadlines !== !1 && k.deadline && (k.deadline = l.date.parseDate(k.deadline, "parse_date")), k.progress = Number(k.progress) || 0, this._isAllowedUnscheduledTask(k) && this._set_default_task_timing(k), this._init_task_timing(k), k.start_date && k.end_date && this.correctTaskWorkTime(k), k.$source = [], k.$target = [];
      var p = this.$data.tasksStore.getItem(k.id);
      return p && !U(k.open) && (k.$open = p.$open), k.parent === void 0 && (k.parent = this.config.root_id), k.open && (k.$open = !0), k;
    }, l), getConfig: function() {
      return l.config;
    } }), h = l.createDatastore({ name: "link", initItem: j(function(k) {
      return this.defined(k.id) || (k.id = this.uid()), k;
    }, l) });
    function _(k) {
      var f = l.isTaskVisible(k);
      if (!f && l.isTaskExists(k)) {
        var p = l.getParent(k);
    ...
    function g(k, f) {
      return f.indexOf(String(k)) === -1 && f.indexOf(Number(k)) === -1;
    }
    function v(k) {
      if (l.isTaskExists(k.source)) {
        for (var f = l.getTask(k.source), p = 0; p < f.$source.length; p++) if (f.$source[p] == k.id) {
          f.$source.splice(p, 1);
          break;
        }
      }
      if (l.isTaskExists(k.target)) {
        var y = l.getTask(k.target);
        for (p = 0; p < y.$target.length; p++) if (y.$target[p] == k.id) {
          y.$target.splice(p, 1);
          break;
        }
      }
    ...
    function m() {
      for (var k = null, f = l.$data.tasksStore.getItems(), p = 0, y = f.length; p < y; p++) (k = f[p]).$source = [], k.$target = [];
    ...
    function b(k) {
      var f = k.source, p = k.target;
      for (var y in k.events) (function($, x) {
    ...
    l.attachEvent("onDestroy", function() {
    ...
    }), l.attachEvent("onLinkValidation", function(k) {
      if (l.isLinkExists(k.id) || k.id === "predecessor_generated") return !0;
      for (var f = l.getTask(k.source).$source, p = 0; p < f.length; p++) {
        var y = l.getLink(f[p]), $ = k.source == y.source, x = k.target == y.target, w = k.type == y.type;
        if ($ && x && w) return !1;
      }
      return !0;
    }), c.attachEvent("onBeforeRefreshAll", function() {
      if (!c._skipTaskRecalculation) for (var k = c.getVisibleItems(), f = 0; f < k.length; f++) {
        var p = k[f];
        p.$index = f, p.$local_index = l.getTaskIndex(p.id), l.resetProjectDates(p);
      }
    }), c.attachEvent("onFilterItem", function(k, f) {
      ...
      if (l.config.start_date && l.config.end_date) {
        ...
        if (p = l.config.start_date.valueOf(), y = l.config.end_date.valueOf(), +f.start_date > y || +f.end_date < +p) return !1;
      }
      return !0;
    }), c.attachEvent("onIdChange", function(k, f) {
      l._update_flags(k, f);
      var p = l.getTask(f);
      c.isSilent() || (p.$split_subtask || p.rollup) && l.eachParent(function(y) {
        l.refreshTask(y.id);
      }, f);
    }), c.attachEvent("onAfterUpdate", function(k) {
      if (l._update_parents(k), l.getState("batchUpdate").batch_update) return !0;
      var f = c.getItem(k);
      f.$source || (f.$source = []);
      for (var p = 0; p < f.$source.length; p++) h.refresh(f.$source[p]);
      for (f.$target || (f.$target = []), p = 0; p < f.$target.length; p++) h.refresh(f.$target[p]);
    }), c.attachEvent("onBeforeItemMove", function(k, f, p) {
      return !Ut(k, l, c) || (console.log("The placeholder task cannot be moved to another position."), !1);
    }), c.attachEvent("onAfterItemMove", function(k, f, p) {
      var y = l.getTask(k);
      this.getNextSibling(k) !== null ? y.$drop_target = this.getNextSibling(k) : this.getPrevSibling(k) !== null ? y.$drop_target = "next:" + this.getPrevSibling(k) : y.$drop_target = "next:null";
    }), c.attachEvent("onStoreUpdated", function(k, f, p) {
      if (p == "delete" && l._update_flags(k, null), !l.$services.getService("state").getState("batchUpdate").batch_update) {
        ...
        p == "add" || p == "move" || p == "delete" ? l.$layout && (this.$config.name != "task" || p != "add" && p != "delete" || this._skipTaskRecalculation != "lightbox" && (this._skipTaskRecalculation = !0), l.$layout.resize()) : k || h.refresh();
      }
    }), h.attachEvent("onAfterAdd", function(k, f) {
      h.sync_link(f);
    }), h.attachEvent("onAfterUpdate", function(k, f) {
      m();
    }), h.attachEvent("onAfterDelete", function(k, f) {
      v(f);
    }), h.attachEvent("onAfterSilentDelete", function(k, f) {
      v(f);
    }), h.attachEvent("onBeforeIdChange", function(k, f) {
      v(l.mixin({ id: k }, l.$data.linksStore.getItem(f))), h.sync_link(l.$data.linksStore.getItem(f));
    }), h.attachEvent("onFilterItem", function(k, f) {
      if (!l.config.show_links) return !1;
      var p = _(f.source), y = _(f.target);
      return !(!p || !y || l._isAllowedUnscheduledTask(l.getTask(f.source)) || l._isAllowedUnscheduledTask(l.getTask(f.target))) && l.callEvent("onBeforeLinkDisplay", [k, f]);
    }), u = {}, l.attachEvent("onBeforeTaskDelete", function(k, f) {
      return u[k] = Me.getSubtreeLinks(l, k), !0;
    }), l.attachEvent("onAfterTaskDelete", function(k, f) {
      u[k] && l.$data.linksStore.silent(function() {
        for (var p in u[k]) l.isLinkExists(p) && l.$data.linksStore.removeItem(p), v(u[k][p]);
        u[k] = null;
      });
    }), l.attachEvent("onAfterLinkDelete", function(k, f) {
      ...
      if (l.isTaskExists(k.source)) {
        var f = l.getTask(k.source);
        f.$source = f.$source || [], g(k.id, f.$source) && f.$source.push(k.id);
      }
      if (l.isTaskExists(k.target)) {
        var p = l.getTask(k.target);
        p.$target = p.$target || [], g(k.id, p.$target) && p.$target.push(k.id);
      }
    };
    ...
    l._cached_functions = { cache: {}, mode: !1, critical_path_mode: !1, wrap_methods: function(c, h) {
      ...
      if (_) {
        ...
        h._prefetch_originals[c] = _, h[c] = function() {
          for (var v = new Array(arguments.length), m = 0, b = arguments.length; m < b; m++) v[m] = arguments[m];
          if (g.active) {
            var k = g.get_arguments_hash(Array.prototype.slice.call(v));
            g.cache[c] || (g.cache[c] = {});
            var f = g.cache[c];
            if (g.has_cached_value(f, k)) return g.get_cached_value(f, k);
            var p = _.apply(this, v);
            return g.cache_value(f, k, p), p;
          }
  ...
  var o = new Ma({ en: ya, ar: ha, be: _a, ca: ga, cn: fa, cs: pa, da: ma, de: va, el: ka, es: ba, fa: $a, fi: xa, fr: wa, he: Sa, hr: Ta, hu: Ca, id: Ea, it: Aa, jp: Da, kr: Ia, nb: La, nl: Na, no: Pa, pl: Ra, pt: Ha, ro: Oa, ru: Ba, si: za, sk: ja, sv: Fa, tr: Wa, ua: Va });
...
function qa(t) {
  ...
  function i(k, f) {
    var p = k.callback;
    v.hide(k.box), e = k.box = null, p && p(f);
  }
  function a(k) {
    if (e) {
      var f = k.which || k.keyCode, p = !1;
      if (m.keyboard) {
        if (f == 13 || f == 32) {
          var y = k.target || k.srcElement;
          it(y).indexOf("gantt_popup_button") > -1 && y.click ? y.click() : (i(e, !0), p = !0);
        }
        f == 27 && (i(e, !1), p = !0);
      }
      return p ? (k.preventDefault && k.preventDefault(), !(k.cancelBubble = !0)) : void 0;
    }
  }
  var s = Et(t.$root) || document;
  function r(k) {
    r.cover || (r.cover = document.createElement("div"), r.cover.onkeydown = a, r.cover.className = "dhx_modal_cover", document.body.appendChild(r.cover)), r.cover.style.display = k ? "inline-block" : "none";
  }
  function o(k, f, p) {
    return "<div " + t._waiAria.messageButtonAttrString(k) + " class='gantt_popup_button " + ("gantt_" + f.toLowerCase().replace(/ /g, "_") + "_button") + "' data-result='" + p + "' result='" + p + "' ><div>" + k + "</div></div>";
  }
  function l() {
    for (var k = [].slice.apply(arguments, [0]), f = 0; f < k.length; f++) if (k[f]) return k[f];
  }
  function d(k, f, p) {
    var y = k.tagName ? k : function(w, T, S) {
    ...
    k.hidden || r(!0), document.body.appendChild(y);
    ...
    return k.position == "top" ? y.style.top = "-3px" : y.style.top = x + "px", y.style.left = $ + "px", y.onkeydown = a, v.focus(y), k.hidden && v.hide(y), t.callEvent("onMessagePopup", [y]), y;
  }
  function u(k) {
    return d(k, !0, !1);
  }
  ...
  function _(k, f, p) {
    return typeof k != "object" && (typeof f == "function" && (p = f, f = ""), k = { text: k, type: f, callback: p }), k;
  }
  function g(k, f, p, y) {
    return typeof k != "object" && (k = { text: k, type: f, expire: p, id: y }), k.id = k.id || ut(), k.expire = k.expire || m.expire, k;
  }
  t.event(s, "keydown", a, !0);
  var v = function() {
    var k = _.apply(this, arguments);
    return k.type = k.type || "alert", h(k);
  };
  v.hide = function(k) {
    for (; k && k.getAttribute && !k.getAttribute(n); ) k = k.parentNode;
    k && (k.parentNode.removeChild(k), r(!1), t.callEvent("onAfterMessagePopup", [k]));
  }, v.focus = function(k) {
    setTimeout(function() {
      var f = Vt(k);
  ...
  var m = function(k, f, p, y) {
    switch ((k = g.apply(this, arguments)).type = k.type || "info", k.type.split("-")[0]) {
      case "alert":
        return u(k);
      case "confirm":
        return c(k);
      ...
      default:
        return function($) {
          m.area || (m.area = document.createElement("div"), m.area.className = "gantt_message_area", m.area.style[m.position] = "5px"), Z(m.area, document.body) || document.body.appendChild(m.area), m.hide($.id);
          var x = document.createElement("div");
          return x.innerHTML = "<div>" + $.text + "</div>", x.className = "gantt-info gantt-" + $.type, x.onclick = function() {
            m.hide($.id), $ = null;
          }, t._waiAria.messageInfoAttr(x), m.position == "bottom" && m.area.firstChild ? m.area.insertBefore(x, m.area.firstChild) : m.area.appendChild(x), $.expire > 0 && (m.timers[$.id] = window.setTimeout(function() {
            m && m.hide($.id);
          }, $.expire)), m.pull[$.id] = x, x = null, $.id;
        }(k);
    }
  };
  m.seed = (/* @__PURE__ */ new Date()).valueOf(), m.uid = ut, m.expire = 4e3, m.keyboard = !0, m.position = "top", m.pull = {}, m.timers = {}, m.hideAll = function() {
    for (var k in m.pull) m.hide(k);
  }, m.hide = function(k) {
    var f = m.pull[k];
    f && f.parentNode && (window.setTimeout(function() {
      f.parentNode.removeChild(f), f = null;
    }, 2e3), f.className += " hidden", m.timers[k] && window.clearTimeout(m.timers[k]), delete m.pull[k]);
  };
  var b = [];
  return t.attachEvent("onMessagePopup", function(k) {
    b.push(k);
  }), t.attachEvent("onAfterMessagePopup", function(k) {
    for (var f = 0; f < b.length; f++) b[f] === k && (b.splice(f, 1), f--);
  }), t.attachEvent("onDestroy", function() {
    r.cover && r.cover.parentNode && r.cover.parentNode.removeChild(r.cover);
    for (var k = 0; k < b.length; k++) b[k].parentNode && b[k].parentNode.removeChild(b[k]);
    b = null, m.area && m.area.parentNode && m.area.parentNode.removeChild(m.area), m = null;
  }), { alert: function() {
    var k = _.apply(this, arguments);
    return k.type = k.type || "confirm", u(k);
  }, confirm: function() {
    var k = _.apply(this, arguments);
    return k.type = k.type || "alert", c(k);
  }, message: m, modalbox: v };
}
function fn(t, n) {
  var e = this.$config[t];
...
var Ka = /* @__PURE__ */ function(t) {
  return function(n) {
    ...
    function a(h) {
      h = h || window.event;
      var _ = n.locate(h), g = r(h, e.click), v = !0;
    ...
    function s(h) {
      var _ = (h = h || window.event).target || h.srcElement, g = n.locate(_), v = n.locate(_, n.config.link_attribute), m = !n.checkEvent("onContextMenu") || n.callEvent("onContextMenu", [g, v, h]);
    ...
    function r(h, _) {
      for (var g = h.target || h.srcElement, v = []; g; ) {
        ...
        if (m) {
          m = m.split(" ");
          for (var b = 0; b < m.length; b++) if (m[b] && _[m[b]]) for (var k = _[m[b]], f = 0; f < k.length; f++) k[f].root && !t.isChildOf(g, k[f].root) || v.push(k[f].handler);
        }
        g = g.parentNode;
      }
      return v;
    ...
    function l(h) {
      h = h || window.event;
      var _ = n.locate(h), g = r(h, e.doubleclick), v = !n.checkEvent("onTaskDblClick") || _ === null || n.callEvent("onTaskDblClick", [_, h]);
      if (v) {
        if (!o(g, h, _)) return;
    ...
    return { reset: c, global: function(h, _, g) {
      i(h, _, g, null);
    }, delegate: i, detach: function(h, _, g, v) {
      if (e[h] && e[h][_]) {
        for (var m = e[h], b = m[_], k = 0; k < b.length; k++) b[k].root == v && (b.splice(k, 1), k--);
        b.length || delete m[_];
      }
    }, callHandler: function(h, _, g, v) {
      var m = e[h][_];
      if (m) for (var b = 0; b < m.length; b++) (g || m[b].root) && m[b].root !== g || m[b].handler.apply(this, v);
...
var Za = function(t) {
  var n = /* @__PURE__ */ function(e) {
    ...
    function r(o, l, d) {
      ...
      var v = l.filter;
      return d && d.setAttribute(e.config.layer_attribute, !0), a[o] = { render_item: function(m, b, k, f, p) {
        if (b = b || d, !v || v(m)) {
          var y = f || s(l), $ = p || (y ? y.$getConfig() : null), x = k;
          !x && $ && $.smart_rendering && (x = y.getViewPort());
          var w = null;
          !Ft(e) && (h || g) && x ? (g ? g(m, x, y, $, e) : pn(x, h(m, y, $, e))) && (w = u.call(e, m, y, $, x)) : w = u.call(e, m, y, $, x), this.append(m, w, b);
          var T = b.nodeType == 11;
          _ && !T && w && _.call(e, m, w, y);
        } else this.remove_item(m.id);
      }, clear: function(m) {
        l.renderer && l.renderer.clear && l.renderer.clear(), this.rendered = i[o] = {}, l.append || this.clear_container(m);
      }, clear_container: function(m) {
        (m = m || d) && (m.innerHTML = "");
      }, get_visible_range: function(m) {
        var b, k, f = s(l), p = f ? f.$getConfig() : null;
        return p && p.smart_rendering && (b = f.getViewPort()), f && b && (typeof l.renderer == "function" ? k = Q(e, f, 0, m, b) : l.renderer && l.renderer.getVisibleRange && (k = l.renderer.getVisibleRange(e, f, p, m, b))), k || (k = { start: 0, end: m.count() }), k;
        ...
        for (var $ = 0, x = m.length; $ < x; $++) this.render_item(m[$], k, f, p, y);
        b.appendChild(k, b);
        var w = {};
        ...
        var k = s(l), f = k ? k.$getConfig() : null;
        if (k && k.$getConfig().smart_rendering && !Ft(e) && this.rendered && (h || g)) {
          b = b || d;
          ...
          for (var S = {}, C = (T = 0, m.length); T < C; T++) {
            ...
            w[E.id] = !1, A && A.parentNode ? (g ? g(E, y, k, f, e) : pn(y, h(E, k, f, e))) ? (c && c.call(e, E, A, k, f, y), this.restore(E, p)) : w[E.id] = !0 : (S[m[T].id] = !0, this.render_item(m[T], p, y, k, f));
          }
          for (var T in w) w[T] && this.hide(T);
          if (p.childNodes.length && b.appendChild(p, b), _) {
            var I = {};
            for (var T in this.rendered) x[T] && !S[T] || (I[T] = this.rendered[T], _.call(e, $[T], this.rendered[T], k));
          }
        }
      }, append: function(m, b, k) {
        this.rendered && (b ? (this.rendered[m.id] && this.rendered[m.id].parentNode ? this.replace_item(m.id, b) : k.appendChild(b), this.rendered[m.id] = b) : this.rendered[m.id] && this.remove_item(m.id));
      }, replace_item: function(m, b) {
        var k = this.rendered[m];
        k && k.parentNode && k.parentNode.replaceChild(b, k), this.rendered[m] = b;
        ...
        var k = this.rendered[m.id];
        k ? k.parentNode || this.append(m, k, b || d) : this.render_item(m, b || d);
  ...
  function i(s, r, o) {
    r.$getConfig(), s.getVisibleItems().forEach(function(l) {
      var d = function(u, c, h, _) {
        ...
        var g = vn(_.getTask(u.source), c), v = vn(_.getTask(u.target), c);
        if (!g || !v) return null;
        var m = 100, b = Math.min(g.left, v.left) - m, k = Math.max(g.left + g.width, v.left + v.width) + m, f = Math.min(g.top, v.top) - m, p = Math.max(g.top + g.height, v.top + v.height) + m;
        return { top: f, height: p - f, bottom: p, left: b, width: k - b, right: k };
      }(l, r, 0, o);
...
function Zn(t, n, e, i, a) {
  ...
  return _ > g && (v = g, g = _, _ = v), c += -100, h += 100, _ += -100, g += 100, !(n.x > h && n.x > g) && !(n.x_end < c && n.x_end < _);
}
function Qa(t, n) {
  if (t.view) {
    var e = t.view;
...
var wt = function() {
  ...
  return t.prototype.destructor = function() {
    ...
    typeof n == "string" && (n = [n]);
    var s = {};
    for (a = 0; a < n.length; a++) xt(i, n[a]), s[n[a]] = !0;
    e._currentBorders = s;
...
var Qn = function(t) {
  ...
  return F(e, t), e.prototype.init = function(a) {
    ...
    for (var a = this._getLinkedViews(), s = this._isVertical() ? "gantt_layout_outer_scroll gantt_layout_outer_scroll_vertical" : "gantt_layout_outer_scroll gantt_layout_outer_scroll_horizontal", r = 0; r < a.length; r++) xt(a[r].$view || a[r].getNode(), s);
    ...
    for (var a, s = 0, r = 0, o = this._isHorizontal(), l = this._getLinkedViews(), d = o ? "scrollWidth" : "scrollHeight", u = o ? "contentX" : "contentY", c = o ? "x" : "y", h = this._getScrollOffset(), _ = 0; _ < l.length; _++) if ((a = l[_]) && a.$content && a.$content.getSize && !a.$config.hidden) {
      ...
      if (g = v.hasOwnProperty(d) ? v[d] : v[u], h) v[u] > v[c] && v[u] > s && g > v[c] - h + 2 && (s = g + (o ? 0 : 2), r = v[c]);
        ...
        var m = Math.max(v[u] - g, 0);
        (g += m) > Math.max(v[c] - m, 0) && g > s && (s = g, r = v[c]);
      }
    }
    ...
    if (this._checkWheelTarget(s)) {
      ...
      if (v !== !1 && n.indexOf(v) >= 0 && (!a[v] || a.deltaX || a.wheelDeltaX || (_ = 2 * g, g = 0)), _ && Math.abs(_) > Math.abs(g)) {
        if (this._isVertical()) return;
        if (r.x || !this.$scroll_hor || !this.$scroll_hor.offsetWidth) return !0;
        var m = _ / -40, b = this._oldLeft, k = b + 30 * m;
        if (this.scrollHorizontally(k), this.$scroll_hor.scrollLeft = k, b == this.$scroll_hor.scrollLeft) return !0;
  ...
  function l(c, h, _) {
    var g = Math.floor(500 / c) || 1, v = Math.floor(500 / h) || 1, m = document.createElement("canvas");
    m.height = h * v, m.width = c * g;
    var b = m.getContext("2d");
    return function(f, p, y, $, x, w) {
      var T = x.createImageData(p * $, f * y);
      T.imageSmoothingEnabled = !1;
      for (var S = 1 * w.rightBorderWidth, C = a(w.rightBorderColor), E = 0, A = 0, I = 0, M = 1; M <= $; M++) for (E = M * p - 1, I = 0; I < S; I++) for (A = 0; A < f * y; A++) k(E - I, A, C, T);
      var D = 1 * w.bottomBorderWidth, L = a(w.bottomBorderColor);
      A = 0;
      for (var N = 1; N <= y; N++) for (A = N * f - 1, I = 0; I < D; I++) for (E = 0; E < p * $; E++) k(E, A - I, L, T);
      x.putImageData(T, 0, 0);
    }(h, c, v, g, b, _), m.toDataURL();
    function k(f, p, y, $) {
      var x = 4 * (p * (c * g) + f);
    ...
    a.$config.fetchTasks && a.silent(function() {
      ...
      for (var l in a.eachItem(function(d) {
        if (d.$role != "task") {
          ...
          u.sort(function(h, _) {
            const g = a.pull, v = g[`${h.task_id}_${h.resource_id}`], m = g[`${_.task_id}_${_.resource_id}`];
            return v && m ? v.$local_index - m.$local_index : 0;
...
Ye.prototype = { init: function(t) {
  ...
  for (var h = u; h < c && t.trace_x[h]; h++) {
    ...
    t.height;
    var v = t.left[h], m = "", b = "", k = "";
    if (g) {
      m = "width:" + g + "px;" + (a.smart_scales ? "position:absolute;left:" + v + "px" : "");
      const p = this.getViewPort(), y = (a.scales[i] || {}).sticky;
      let $ = "";
      const x = 70;
      if (y !== !1 && g > x || y === !0) {
        if (v < p.x && v + g / 2 - x / 2 < p.x) $ = ` style='position:absolute;left: ${p.x - v + 10}px;' `;
        else if (v + g / 2 + x / 2 > p.x_end && g > x) {
          let w = p.x_end - v - 10, T = "-100%";
          w < x && (w = x, T = `-${w}px`), $ = ` style='position:absolute;left: ${w}px;transform: translate(${T},0);' `;
        }
      }
      k = "gantt_scale_cell" + (h == t.count - 1 ? " gantt_last_cell" : ""), (b = l.call(this, o)) && (k += " " + b);
      var f = `<div class='${k}' ${this.$gantt._waiAria.getTimelineCellAttr(_)} style='${m}'><span ${$}>${_}</span></div>`;
      r.push(f);
    }
  }
  return r.join("");
  ...
  if (_ <= 0) return u;
  const v = Math.round(r * _), m = d.valueOf() + 1e3 * (c + v);
  return new Date(m);
...
const os = function(t) {
  ...
  return F(n, t), H(n.prototype, { init: function(e) {
    ...
    for (let c = 0; c < r.length; c++) {
      ...
      let m = e._sort && _.name == e._sort.name ? `<div data-column-id="${_.name}" class="gantt_sort gantt_${e._sort.direction}"></div>` : "", b = ["gantt_grid_head_cell", "gantt_grid_head_" + _.name, h ? "gantt_last_cell" : "", s.grid_header_class ? s.grid_header_class(_.name, _) : ""].join(" "), k = "width:" + (g - (h ? 1 : 0)) + "px;", f = _.label || d["column_" + _.name] || d[_.name];
      f = f || "";
      const p = "<div class='" + b + "' style='" + k + "' " + e._waiAria.gridScaleCellAttrString(_, f) + " data-column-id='" + _.name + "' column_id='" + _.name + "' data-column-name='" + _.name + "' data-column-index='" + c + "'>" + f + m + "</div>";
      o.push(p);
    }
    ...
    switch (h) {
      ...
      default:
        if (t.isVisible()) _ = !0;
          ...
          var v = r.modifiers, m = v.alt || v.ctrl || v.meta || v.shift;
          v.alt || o.key === "Meta" || m && s.getCommandHandler(r, "taskCell") || d && !t.isVisible() && (e.startEdit(l.id, l.columnName), _ = !0);
        }
    }
    return !_;
  });
...
function fs(t) {
  ...
  function o(l, d, u) {
    var c = function(m, b, k) {
      var f = [];
      return [...new Set(b)].forEach(function(p) {
        var y = i(k).parse(p);
        y && (y.target = m, y.id = "predecessor_generated", t.isLinkAllowed(y) && (y.id = void 0, f.push(y)));
      }), f;
    }(l.id, d, u), h = {};
...
function ks(t) {
  ...
  var i = { init: vs, createEditors: function(a) {
    function s(c, h) {
      var _ = a.$getConfig(), g = function(b, k) {
        for (var f = a.$getConfig(), p = a.getItemTop(b), y = a.getItemHeight(b), $ = a.getGridColumns(), x = 0, w = 0, T = 0, S = 0; S < $.length; S++) {
          if ($[S].name == k) {
            T = $[S].width;
            break;
          }
          f.rtl ? w += $[S].width : x += $[S].width;
        }
        return f.rtl ? { top: p, right: w, height: y, width: T } : { top: p, left: x, height: y, width: T };
      ...
      var m = function(b, k) {
        for (var f = b.getGridColumns(), p = 0; p < f.length; p++) if (f[p].name == k) return p;
        return 0;
      }(a, h);
      return v.setAttribute("data-column-index", m), t._waiAria.inlineEditorAttr(v), _.rtl ? v.style.cssText = ["top:" + g.top + "px", "right:" + g.right + "px", "width:" + g.width + "px", "height:" + g.height + "px"].join(";") : v.style.cssText = ["top:" + g.top + "px", "left:" + g.left + "px", "width:" + g.width + "px", "height:" + g.height + "px"].join(";"), v;
    }
    var r = ms(t), o = [], l = [], d = null, u = { _itemId: null, _columnName: null, _editor: null, _editorType: null, _placeholder: null, locateCell: function(c) {
      ...
      if (!v) return;
      var m = a.$getConfig().editor_types[v.type], b = s(_.id, _.columnName);
      a.$grid_data.appendChild(b);
      const k = (function() {
        this._editor = m, this._placeholder = b, this._itemId = _.id, this._columnName = _.columnName, this._editorType = v.type;
        var p = n.getMapping();
        p.onShow && p.onShow(this, b, a), b._onReMount = (function() {
          this.setValue();
        }).bind(this);
      }).bind(this), f = m.show(_.id, g, v, b);
      if (f && f.then) return f.then(() => {
        k();
      });
      k();
    }, setValue: function() {
      var c, h = this.getState(), _ = h.id, g = h.columnName, v = a.getColumn(g), m = d.getItem(_), b = this.getEditorConfig(g);
      if (b) {
        if (t._lightbox_id) {
          let k = d.getItem(this._itemId);
          b.map_to === "text" ? c = t.getDatastore(t.config.resource_store).getItem(k.resource_id).text : c = b.map_to === "start_date" ? m.start_date === "" || m.start_date === null ? t.getTask(t._lightbox_id).start_date : m.start_date : m[b.map_to];
        } else c = m[b.map_to];
      ...
      if (this.isVisible() && d.exists(this._itemId) && this.isChanged()) {
        ...
        if (d.exists(c)) {
          ...
          if (this.callEvent("onBeforeSave", [v]) !== !1 && (!this._editor.is_valid || this._editor.is_valid(v.newValue, v.id, a.getColumn(h), this._placeholder))) {
            ...
            var m = g.map_to, b = v.newValue;
            m != "auto" ? (_[m] = b, r(_, m, t.config.inline_editors_date_processing), d.updateItem(c)) : this._editor.save(c, a.getColumn(h), this._placeholder), this.callEvent("onSave", [v]);
          }
...
function $e(t) {
  function n(s, r, o) {
    ...
    const v = t.config.baselines && s.baselines && s.baselines.length, m = t.config.baselines && (t.config.baselines.render_mode == "separateRow" || t.config.baselines.render_mode == "individualRow");
    if (v && m && s.bar_height !== "full" && s.bar_height < s.row_height) if (c === d.types.milestone) {
      let S = r.getBarHeight(s.id, !0), C = Math.sqrt(2 * S * S);
      g = Math.floor((C - h) / 2) + 2;
    } else g = 2;
    c == d.types.milestone && (l.left -= Math.round(h / 2), l.width = h);
    var b = document.createElement("div"), k = Math.round(l.width);
...
<path d="M5.58397 1.52543C5.78189 1.22856 6.21811 1.22856 6.41602 1.52543L10.5475 7.72265C10.769 8.05493 10.5308 8.5 10.1315 8.5L1.86852 8.5C1.46917 8.5 1.23097 8.05493 1.45249 7.72265L5.58397 1.52543Z" fill="var(--dhx-gantt-progress-handle-background)" stroke="var(--dhx-gantt-progress-handle-border)"/>
    ...
    var y = ["left:" + l.left + "px", "top:" + (g + l.top) + "px", "height:" + h + "px", "line-height:" + Math.max(h < 30 ? h - 2 : h, 0) + "px", "width:" + k + "px"];
    ...
    return (w || T) && (d.drag_resize && !t.isSummaryTask(s) && c != d.types.milestone && a(b, "gantt_task_drag", s, function(S) {
      ...
      var A = !1;
      return x.link_source_id && d.touch && (A = !0), E.style.display = A ? "block" : "", C.appendChild(E), C;
    }, d, _)), b;
  }
  function e(s, r, o, l) {
    if (!r) return null;
...
function bs(t) {
  function n(s, r, o) {
    ...
    const y = document.createElement("div"), $ = r.getItemPosition(s, s.$auto_start_date || s.start_date, s.$auto_end_date || s.end_date);
    y.classList.add("gantt_task_line_actual", "gantt_task_line", "gantt_project"), y.style.cssText = ["position:absolute", "left:" + $.left + "px", "top:16px", "height:8px", "width:" + $.width + "px"].join(";"), b.appendChild(y), d.show_progress && c != d.types.milestone && function(S, C, E, A, I) {
      var M = 1 * S.progress || 0;
      E = Math.max(E - 2, 0);
      var D = document.createElement("div"), L = Math.round(E * M);
...
function ws(t) {
  var n = { current_pos: null, dirs: { left: "left", right: "right", up: "up", down: "down" }, path: [], clear: function() {
    ...
    var g = 2 * d.link_arrow_size, v = this.get_line_type(a, s.$getConfig()), m = l.e_x > l.x;
    if (v.from_start && v.to_start) this.point_to(h.left, g), m ? (this.point_to(h.down, u), this.point_to(h.right, c)) : (this.point_to(h.right, c), this.point_to(h.down, u)), this.point_to(h.right, g);
    else if (!v.from_start && v.to_start) if (u !== 0 && (m = l.e_x > l.x + 2 * g), this.point_to(h.right, g), m) c -= g, this.point_to(h.down, u), this.point_to(h.right, c);
    else {
      c -= 2 * g;
      var b = u > 0 ? 1 : -1;
      this.point_to(h.down, b * (_ / 2)), this.point_to(h.right, c), this.point_to(h.down, b * (Math.abs(u) - _ / 2)), this.point_to(h.right, g);
    }
    else v.from_start || v.to_start ? v.from_start && !v.to_start && (u !== 0 && (m = l.e_x > l.x - 2 * g), this.point_to(h.left, g), m ? (c += 2 * g, b = u > 0 ? 1 : -1, this.point_to(h.down, b * (_ / 2)), this.point_to(h.right, c), this.point_to(h.down, b * (Math.abs(u) - _ / 2)), this.point_to(h.left, g)) : (c += g, this.point_to(h.down, u), this.point_to(h.right, c))) : (this.point_to(h.right, g), m ? (this.point_to(h.right, c), this.point_to(h.down, u)) : (this.point_to(h.down, u), this.point_to(h.right, c)), this.point_to(h.left, g));
    return this.path;
  ...
  return { render: function(a, s, r) {
    ...
    const h = function(m, b) {
      const k = b.link_radius || 4, f = b.link_arrow_size || 6, p = [];
      for (let $ = 0; $ < m.length; $++) {
        const x = m[$], w = m[$ + 1];
        if (!w || b.link_radius <= 1) p.push({ type: "line", data: x });
        else if (x.direction !== w.direction) {
          if (x.size < k || w.size < k) {
            p.push({ type: "line", data: x });
            continue;
          }
          x.size -= k, p.push({ type: "line", data: x });
          ...
          const C = { x: T, y: S, direction: { from: x.direction, to: w.direction }, radius: k };
      ...
      if (y.direction === "right" || y.direction === "left") {
        y.size -= 3 * f / 4;
        let $ = y.direction === "right" ? y.x + y.size : y.x - y.size - f / 2, x = y.y - b.link_line_width / 2 - f / 2 + 1;
        y.direction === "left" ? (x -= 1, $ -= 2) : $ -= 1;
        const w = { x: $, y: x, size: f, direction: y.direction };
        p.push({ type: "line", data: y }), p.push({ type: "arrow", data: w });
      } else p.push({ type: "line", data: y });
      return p;
    }(n.get_lines(c, s).filter((m) => m.size > 0), r), _ = function(m, b, k, f) {
      const p = document.createElement("div");
      return m.forEach((y) => {
        let $;
        y.type === "line" ? $ = n.render_line(y.data, null, b, k.source) : y.type === "corner" ? $ = n.render_corner(y.data, b) : y.type === "arrow" && ($ = n.render_arrow(y.data, f)), p.appendChild($);
      }), p;
    }(h, s, a, r);
      ...
      t.config.process_resource_assignments && d === t.config.resource_property ? (h = l.$role == "task" ? t.getResourceAssignments(l.$resource_id, l.$task_id) : t.getResourceAssignments(l.id), _ = !0) : h = l.$role == "task" ? [] : t.getTaskBy(d, l.id), g = function(w, T, S) {
        ...
        for (let M = 0; M < w.length; M++) {
          ...
          for (I[z.id] = {}; B < P; ) {
            ...
            B = t.date.add(B, E, C), K[W] !== !1 && (A[W] || (A[W] = { tasks: [], assignments: [] }), A[W].tasks.push(L), S && A[W].assignments.push(D));
          }
        }
        return A;
      }(h, u, _);
      const v = u.unit, m = u.step, b = [];
      let k, f, p, y, $;
      const x = c.$getConfig();
      for (let w = 0; w < u.trace_x.length; w++) k = new Date(u.trace_x[w]), f = t.date.add(k, m, v), $ = g[k.valueOf()] || {}, p = $.tasks || [], y = $.assignments || [], p.length || x.resource_render_empty_cells ? b.push({ start_date: k, end_date: f, tasks: p, assignments: y }) : b.push(null);
      return b;
    }(e, i, a, s), o;
  };
}
...
function Es(t) {
  ...
  function r(l, d, u, c, h, _, g) {
    ...
    var m = function(b, k, f, p) {
      for (var y = k.getScale(), $ = document.createElement("div"), x = Ct(y, p), w = x.start; w <= x.end; w++) {
        var T = y.trace_x[w], S = y.trace_x[w + 1] || t.date.add(T, y.step, y.unit), C = y.trace_x[w].valueOf(), E = Math.min(b[C] / f, 1) || 0;
        if (E < 0) return null;
        var A = Math.min(b[S.valueOf()] / f, 1) || 0, I = Ts(E, T, S, k);
        I && $.appendChild(I);
        var M = Cs(E, A, k.posFromDate(S));
        M && $.appendChild(M);
      }
      return $;
    }(u, h, _, g);
  ...
  function o(l, d, u, c, h, _, g) {
    var v = h.histogram_cell_class(_.start_date, _.end_date, l, _.tasks, _.assignments), m = h.histogram_cell_label(_.start_date, _.end_date, l, _.tasks, _.assignments), b = h.histogram_cell_allocated(_.start_date, _.end_date, l, _.tasks, _.assignments), k = g.getItemHeight(l.id) - 1;
    if (v || m) {
      var f = document.createElement("div");
      return f.className = ["gantt_histogram_cell", v].join(" "), f.setAttribute(g.$config.item_attribute, l.id), f.style.cssText = ["left:" + d.left + "px", "width:" + d.width + "px", "height:" + k + "px", "line-height:" + k + "px", "top:" + (d.top + 1) + "px"].join(";"), m && (m = "<div class='gantt_histogram_label'>" + m + "</div>"), b && (m = "<div class='gantt_histogram_fill' style='height:" + 100 * Math.min(b / u || 0, 1) + "%;'></div>" + m), m && (f.innerHTML = m), f;
    }
    return null;
  }
  return { render: function(l, d, u, c) {
    var h = d.$getTemplates(), _ = d.getScale(), g = n(l, u.resource_property, _, d), v = [], m = {}, b = l.capacity || d.$config.capacity || 24;
    e[l.id] = {}, i[l.id] = null, a[l.id] = null;
    for (var k = !!c, f = Ct(_, c), p = f.start; p <= f.end; p++) {
      var y = g[p];
      if (y && (!k || Wt(p, _, c, t))) {
        var $ = h.histogram_cell_capacity(y.start_date, y.end_date, l, y.tasks, y.assignments);
        m[y.start_date.valueOf()] = $ || 0;
    ...
    if (v.length) {
      ...
      var C = r(l, x, m, 0, d, b, c);
      C && (T.appendChild(C), a[l.id] = C), i[l.id] = T;
    }
    return T;
  }, update: function(l, d, u, c, h) {
    var _ = u.$getTemplates(), g = u.getScale(), v = n(l, c.resource_property, g, u), m = l.capacity || u.$config.capacity || 24, b = {}, k = !!h, f = Ct(g, h), p = {};
    if (e && e[l.id]) for (var y in e[l.id]) p[y] = y;
    for (var $ = f.start; $ <= f.end; $++) {
      var x = v[$];
      if (p[$] = !1, x) {
...
const Ls = { init: function(t, n) {
  ...
  if (n.$config.bind && t.getDatastore(n.$config.bind)) {
    ...
    t.defined(n.$getConfig().dnd_sensitivity) && (i.config.sensitivity = n.$getConfig().dnd_sensitivity), i.attachEvent("onBeforeDragStart", t.bind(function(o, l) {
      ...
      var m = g.getItem(i.config.id), b = i._getTargetY(l), k = i._getTaskByY(b, g.getIndexById(m.id));
      function f(A, I) {
        return !g.isChildOf(p.id, I.id) && (A.$level == I.$level || _.order_branch_free);
      }
      if (g.exists(k) || (k = i.lastTaskOfLevel(_.order_branch_free ? m.$level : 0)) == i.config.id && (k = null), g.exists(k)) {
        var p = g.getItem(k), y = n.getItemTop(p.id), $ = n.getItemHeight(p.id);
        if (y + $ / 2 < b) {
          var x = g.getIndexById(p.id), w = g.getNext(p.id), T = g.getItem(w);
          if (r(w)) {
          ...
          if (T) {
            ...
            p = T;
          } else if (w = g.getIdByIndex(x), T = g.getItem(w), r(w) && (S = g.getPrev(T.id), T = g.getItem(S)), f(T, m) && T.id != m.id) return void g.move(m.id, -1, g.getParent(T.id));
        } else if (_.order_branch_free && p.id != m.id && f(p, m) && !r(p.id)) {
          if (!g.hasChild(p.id)) return p.$open = !0, void g.move(m.id, -1, p.id);
          if (g.getIndexById(p.id) || $ / 3 < b) return;
        }
        x = g.getIndexById(p.id), S = g.getIdByIndex(x - 1);
        for (var C = g.getItem(S), E = 1; (!C || C.id == p.id) && x - E >= 0; ) S = g.getIdByIndex(x - E), C = g.getItem(S), E++;
...
function bn(t, n, e, i, a, s) {
  ...
  if (n !== a.$getRootId()) {
    ...
    if (l === s) {
      ...
      var u = xe(t, n, s, a), c = yn(t, n, s, a);
      r = e < 0.5 ? u : c;
    }
  } else {
    var h = a.$getRootId(), _ = a.getChildren(h);
    r = nt.createDropTargetObject(), r = _.length && i >= 0 ? xe(t, function(g) {
      for (var v = g.getNext(); g.exists(v); ) {
        var m = g.getNext(v);
        if (!g.exists(m)) return v;
        v = m;
      }
      return null;
    }(a), s, a) : yn(t, h, s, a);
...
const xn = { removeLineHighlight: function(t) {
  ...
  a || ((a = document.createElement("div")).className = "gantt_drag_marker gantt_grid_dnd_marker", a.innerHTML = "<div class='gantt_grid_dnd_marker_line'></div>", a.style.pointerEvents = "none"), t.child ? function(s, r, o) {
    ...
    var l = function(c, h) {
      var _ = h.$config.rowStore, g = { x: 0, y: 0 }, v = h.$grid_data.querySelector(".gantt_tree_indent"), m = 15, b = 0;
      v && (m = v.offsetWidth);
      var k = 40;
      ...
      return g.x = k + b * m, g.width = we(h.$gantt, Math.max(h.$grid_data.offsetWidth - g.x, 0), g.x), $n(g, h);
  ...
  function r(o) {
    var l, d = function(g) {
      var v = dt(g, n.$grid_data).y, m = n.$config.rowStore;
      document.doctype || (v += window.scrollY), v = v || 0;
      var b = n.$state.scrollTop || 0, k = t.$grid_data.getBoundingClientRect().height + b + window.scrollY, f = b, p = n.getItemIndexByTopPosition(n.$state.scrollTop);
      if (m.exists(p) || (p = m.countVisible() - 1), p < 0) return m.$getRootId();
      var y = m.getIdByIndex(p), $ = n.$state.scrollTop / n.getItemHeight(y), x = $ - Math.floor($);
      x > 0.1 && x < 0.9 && (k -= n.getItemHeight(y) * x, f += n.getItemHeight(y) * (1 - x));
      const w = Y(n.$grid_data), T = w.y + w.height, S = i.config.marker.offsetHeight;
      v + S + window.scrollY >= k && (i.config.marker.style.top = T - S + "px"), v >= k ? v = k : v <= f && (v = f, i.config.marker.style.top = w.y + "px");
    ...
    return document.doctype || (_ += window.scrollY), d !== c.$getRootId() && (u = (_ - n.getItemTop(d)) / n.getItemHeight(d)), h ? (l = bn(i.config.id, d, u, _, c, i.config.level)) && l.targetParent && s(l.targetParent) && (d = c.getPrevSibling(l.targetParent), l = bn(i.config.id, d, u, _, c, i.config.level)) : l = function(g, v, m, b, k) {
      var f;
      if (v !== k.$getRootId()) f = m < 0.25 ? nt.prevSiblingTarget(g, v, k) : !(m > 0.6) || k.hasChild(v) && k.getItem(v).$open ? nt.firstChildTarget(g, v, k) : nt.nextSiblingTarget(g, v, k);
      else {
        var p = k.$getRootId();
        f = k.hasChild(p) && b >= 0 ? nt.lastChildTarget(g, p, k) : nt.firstChildTarget(g, p, k);
      }
      return f;
    ...
    return t = function(i, a) {
      ...
      return { drag: null, dragMultiple: {}, _events: { before_start: {}, before_finish: {}, after_finish: {} }, _handlers: {}, init: function() {
        ...
        if (o.mode) {
          ...
          o.pos = d;
          const k = a.getTask(o.id);
          var u = a.dateFromPos(d.x, this._getPositioningContext(k));
          ...
          if (this._handlers[o.mode]) {
            if (o.mode === l.drag_mode.move) {
              ...
              var _ = !1;
              if (a.isSummaryTask(k) && a.config.drag_project) {
                var g = {};
              ...
              if (v !== void 0 && (c = v), this._update_item_on_move(c, o.id, o.mode, o, r), v === void 0) {
                ...
                f = u - a.dateFromPos($, this._getPositioningContext(o.obj));
              }
              for (var m in h) {
                var b = h[m];
...
var Hs = function(t, n) {
  ...
  function g(f, p) {
    var y, $ = _.getPosition(f), x = function(I) {
      var M = 0, D = 0;
      return I && (M = I.offsetWidth || 0, D = I.offsetHeight || 0), { width: M, height: D };
    }(p), w = { right: (y = n.$root).offsetWidth, bottom: y.offsetHeight }, T = n.config.tooltip_offset_x || 10, S = n.config.tooltip_offset_y || 10, C = n.config.scroll_size || 18, E = n.$container.getBoundingClientRect().y + window.scrollY, A = { y: $.y + S, x: $.x + T, bottom: $.y + x.height + S + C, right: $.x + x.width + T + C };
    return A.bottom > w.bottom + E && (A.y = w.bottom + E - x.height - S), A.right > w.right && (A.x = w.right - x.width - T), A;
  }
  function v(f) {
    var p = o();
  ...
  function b(f, p, y, $) {
    ...
    var S = Math.sqrt(Math.pow(y - f, 2) + Math.pow($ - p, 2));
    if (S = Math.max(0, S - 3)) {
      x.className = T.join(" ");
      var C = ($ - p) / (y - f), E = Math.atan(C);
      k(f, y, p, $) == 2 ? E += Math.PI : k(f, y, p, $) == 3 && (E -= Math.PI);
      ...
      if (window.navigator.userAgent.indexOf("MSIE 8.0") != -1) {
        ...
        var N = Math.abs(Math.round(f - y)), P = Math.abs(Math.round($ - p));
        switch (k(f, y, p, $)) {
          case 1:
            M -= P;
            break;
      ...
      L.push("top:" + M + "px"), L.push("left:" + D + "px"), x.style.cssText = L.join(";");
    }
  }
  function k(f, p, y, $) {
    return p >= f ? $ <= y ? 1 : 4 : $ <= y ? 2 : 3;
  }
  _.attachEvent("onBeforeDragStart", n.bind(function(f, p) {
    ...
    if (kt(y, c)) {
      ...
      s = $;
      var x = n.getTask($);
      return n.isReadonly(x) ? (m(), !1) : (this._dir_start = { x: _.config.original_element_sizes.x + _.config.original_element_sizes.width / 2, y: _.config.original_element_sizes.y + _.config.original_element_sizes.height / 2 }, !0);
    }
    return !1;
    ...
    var $ = _.config, x = g(p, $.marker);
    (function(M, D) {
      M.style.left = D.x + "px", M.style.top = D.y + "px";
    })($.marker, x);
    var w = !!kt(p, h), T = i, S = e, C = a;
    let E, A = !0;
    if (E = p.target.shadowRoot && y ? y : n.locate(p), Z(At(p), n.$root) || (w = !1, E = null), w && (A = !kt(p, "task_end_date"), w = !!E), i = E, e = w, a = A, w) {
      const M = kt(p, h).querySelector(`.${c}`);
      if (M) {
        const D = Mn(M, t.$task_bg);
...
const js = { init: function(t) {
  ...
  return t.ext.inlineEditors = a, t.ext._inlineEditors = a, a.init(t), { factory: e, mouseEvents: Xa.init(t), layersApi: i.init(), render: { gridLine: function() {
    return /* @__PURE__ */ function(s) {
      return { render: function(r, o, l, d) {
        for (var u = o.getGridColumns(), c = o.$getTemplates(), h = o.$config.rowStore, _ = [], g = 0; g < u.length; g++) {
          var v, m, b, k = g == u.length - 1, f = u[g];
          ...
          var p = "gantt_cell" + (k ? " gantt_last_cell" : ""), y = [];
          if (f.tree) {
            p += " gantt_cell_tree";
            for (var $ = 0; $ < r.$level; $++) y.push(c.grid_indent(r));
          ...
          var x = "width:" + (f.width - (k ? 1 : 0)) + "px;";
          if (this.defined(f.align)) {
            var w = { right: "flex-end", left: "flex-start", center: "center" }[f.align];
            x += "text-align:" + f.align + ";justify-content:" + w + ";";
          }
          var T = s._waiAria.gridCellAttrString(f, b, r);
          y.push(m), v = "<div class='" + p + "' data-column-index='" + g + "' data-column-name='" + f.name + "' style='" + x + "' " + T + ">" + y.join("") + "</div>", _.push(v);
        }
    ...
    return /* @__PURE__ */ function(s) {
      ...
      function c(g) {
        var v, m = g.$getTemplates();
        return m.task_cell_class !== void 0 ? (v = m.task_cell_class, (console.warn || console.log)("gantt.templates.task_cell_class template is deprecated and will be removed soon. Please use gantt.templates.timeline_cell_class instead.")) : v = m.timeline_cell_class, v;
      }
      function h(g) {
        return g.$getTemplates().timeline_cell_content;
      }
      function _(g, v, m, b, k, f, p, y) {
        var $ = g.width[v], x = "";
        if (Wt(v, g, b, s)) {
          var w = f(m, g.trace_x[v]), T = "";
          if (p && (T = p(m, g.trace_x[v])), y.static_background) {
            var S = !(!w && !T);
            if (!y.static_background_cells || !S) return null;
          }
          if (r[m.id][v]) return o[m.id][v] = v, r[m.id][v];
          var C = document.createElement("div");
          return C.style.width = $ + "px", x = "gantt_task_cell" + (v == k - 1 ? " gantt_last_cell" : ""), w && (x += " " + w), C.className = x, T && (C.innerHTML = T), C.style.position = "absolute", C.style.left = g.left[v] + "px", r[m.id][v] = C, o[m.id][v] = v, C;
        }
        return null;
      }
      return { render: function(g, v, m, b) {
        var k = v.$getTemplates(), f = v.getScale(), p = f.count;
        if (m.static_background && !m.static_background_cells) return null;
        var y, $ = document.createElement("div"), x = c(v), w = h(v);
        if (y = b && m.smart_rendering && !Ft(s) ? Ct(f, b.x) : { start: 0, end: p - 1 }, m.show_task_cells) {
          r[g.id] = {}, o[g.id] = {};
          for (var T = y.start; T <= y.end; T++) {
            var S = _(f, T, g, b, p, x, w, m);
            S && $.appendChild(S);
          }
        }
        const C = v.$config.rowStore, E = C.getIndexById(g.id) % 2 != 0;
        var A = k.task_row_class(g.start_date, g.end_date, g), I = "gantt_task_row" + (E ? " odd" : "") + (A ? " " + A : "");
        if (C.isSelected(g.id) && (I += " gantt_selected"), $.className = I, m.smart_rendering ? ($.style.position = "absolute", $.style.top = v.getItemTop(g.id) + "px", $.style.width = "100%") : $.style.position = "relative", $.style.height = v.getItemHeight(g.id) + "px", g.id == "timeline_placeholder_task") {
          var M = 0;
          g.lastTaskId && (M = v.getItemTop(g.lastTaskId) + v.getItemHeight(g.lastTaskId));
          var D = (g.row_height || v.$task_data.offsetHeight) - M;
          D < 0 && (D = 0), m.smart_rendering && ($.style.top = M + "px"), $.style.height = D + "px";
        }
        return v.$config.item_attribute && ($.setAttribute(v.$config.item_attribute, g.id), $.setAttribute(v.$config.bind + "_id", g.id)), $;
      }, update: function(g, v, m, b, k) {
        var f = m.getScale(), p = f.count, y = c(m), $ = h(m);
        if (b.show_task_cells) {
          r[g.id] || (r[g.id] = {}), o[g.id] || (o[g.id] = {});
          var x = Ct(f, k);
          for (var w in o[g.id]) {
            var T = o[g.id][w];
            (Number(T) < x.start || Number(T) > x.end) && u(g.id, T);
          }
          o[g.id] = {};
          for (var S = x.start; S <= x.end; S++) {
            var C = _(f, S, g, k, p, y, $, b);
        ...
        for (let g in r) l(g);
        r = {}, o = {};
      }, getRectangle: Oe, getVisibleRange: Q, prepareData: xs };
    }(t);
  }, taskBar: function() {
    return function(s) {
      return { render: $e(s), update: null, isInViewPort: Mt, getVisibleRange: Q };
    }(t);
    ...
    return function(s) {
      ...
      function d(c, h, _, g) {
        ...
        const m = r(v, _);
        if (!m) return;
        const b = _.getBarHeight(c.id, h.type == s.config.types.milestone), k = Math.floor((_.getItemHeight(c.id) - b) / 2);
        return m.style.top = g.top + k + "px", m.classList.add("gantt_rollup_child"), m.setAttribute("data-rollup-parent-id", c.id), m;
      }
      function u(c, h) {
        return c + "_" + h;
      }
      return { render: function(c, h, _, g) {
        if (c.rollup !== !1 && c.$rollup && c.$rollup.length) {
          const v = document.createElement("div"), m = s.getTaskPosition(c);
          return g && (g.y = 0, g.y_end = s.$task_bg.scrollHeight), c.$rollup.forEach(function(b) {
            if (!s.isTaskExists(b)) return;
            const k = s.getTask(b);
            if (!l(k, g, h, _)) return;
            const f = d(c, k, h, m);
            f ? (o[u(k.id, c.id)] = f, v.appendChild(f)) : o[u(k.id, c.id)] = !1;
          }), v;
        }
        return !1;
      }, update: function(c, h, _, g, v) {
        const m = document.createElement("div"), b = s.getTaskPosition(c);
        v.y = 0, v.y_end = s.$task_bg.scrollHeight, c.$rollup.forEach(function(k) {
          const f = s.getTask(k), p = u(f.id, c.id);
          let y = l(f, v, _, g);
          if (y !== !!o[p]) if (y) {
            const $ = d(c, f, _, b);
            o[p] = $ || !1;
    ...
    return function(s) {
      const r = $e(s), o = new Wn(s.$data.tasksStore), l = {};
      function d(_, g, v, m, b) {
        let k = !_.hide_bar;
        return m.smart_rendering && k && (k = Mt(_, g, v)), k;
      }
      function u(_, g, v, m, b) {
        if (g.hide_bar) return;
        const k = s.isSummaryTask(g);
        k && s.resetProjectDates(g);
        ...
        let $;
        const x = m.rowHeight, w = v.getBarHeight(f.id, y);
        ...
        if (y) {
          if ($ > x) {
            ...
            const E = un(x), A = (E - x) / 2, I = p.querySelector(".gantt_task_content");
            T = Math.abs(A), S.style.marginLeft = A + "px", C.style.marginRight = A + "px", S.style.height = C.style.height = E + "px", p.style.width = I.style.height = I.style.width = E + "px", p.style.left = v.getItemPosition(f).left - E / 2 + "px";
          }
        } else w + T > x && (T = 0, p.style.height = p.style.lineHeight = S.style.height = C.style.height = x + "px");
        return p.style.top = m.top + T + "px", p.classList.add("gantt_split_child"), k && p.classList.add("gantt_split_subproject"), p;
      }
      function c(_, g) {
        return _ + "_" + g;
      }
      ...
      return { render: function(_, g, v, m) {
        if (h(_, v)) {
          const b = document.createElement("div"), k = s.getTaskPosition(_), f = Tt(s, _.id);
          return s.hasChild(_.id) && s.eachTask(function(p) {
            if (d(p, m, g, v) && (o.isDefaultSplitItem(p) || o.isInlineSplitItem(p))) {
              const y = u(_, p, g, k, f);
        ...
        if (s.isSplitTask(_) && s.hasChild(_.id)) {
          ...
          return s.eachTask(function(p) {
            if (d(p, m, g, v) && o.isInlineSplitItem(p)) {
              const y = u(_, p, g, k, f);
              y ? (l[c(p.id, _.id)] = y, b.appendChild(y)) : l[c(p.id, _.id)] = !1;
            }
          }, _.id), b;
        ...
        return !1;
      }, update: function(_, g, v, m, b) {
        if (h(_, m)) {
          const k = document.createElement("div"), f = s.getTaskPosition(_), p = Tt(s, _.id);
          s.eachTask(function(y) {
            ...
            l[$] && k.appendChild(l[$]), g.innerHTML = "", g.appendChild(k);
          }, _.id);
        }
        if (s.isSplitTask(_) && s.hasChild(_.id)) {
          const k = document.createElement("div"), f = s.getTaskPosition(_), p = Tt(s, _.id);
          s.eachTask(function(y) {
            if (d(y, b, v, m) && o.isInlineSplitItem(y)) {
              const $ = u(_, y, v, f, p);
              $ ? (l[c(y.id, _.id)] = $, k.appendChild($)) : l[c(y.id, _.id)] = !1;
            }
          }, _.id);
        }
      }, isInViewPort: $s, getVisibleRange: Q };
    }(t);
    ...
    return /* @__PURE__ */ function(s) {
      function r(o, l, d, u, c) {
        ...
        let k = ne(s, u, o, v.rowHeight).maxHeight, f = v.top + 1 + b, p = u.getBarHeight(o.id, o.type);
        const y = s.config.baselines.row_height, $ = s.config.baselines.bar_height;
        let x, w;
        switch (s.config.baselines.render_mode) {
          case "separateRow":
            ...
            break;
          case "individualRow":
            x = y * d, f += v.height + x + (y - $) / 2, p = $;
            break;
          default:
            w = 1, c ? (k = ne(s, u, o).maxHeight, b ? k >= m ? w = (k - p) / 2 - 1 - b : (p = un(k), f = v.top, w = Math.abs(p - k) / 2, b = 0) : (k > p && (w = (k - p) / 2 - 1), w -= b), o.bar_height || (w -= 1)) : (o.bar_height && v.rowHeight >= o.bar_height && (w = (v.rowHeight - o.bar_height) / 2 - 1), w += b, o.bar_height || (w += 2), g && (w += 1));
        }
        let T = v.top + k + 1 - b;
    ...
    return function(s) {
      ...
      function l(u, c, h, _, g) {
        var v = h.resource_cell_class(c.start_date, c.end_date, u, c.tasks, c.assignments), m = h.resource_cell_value(c.start_date, c.end_date, u, c.tasks, c.assignments), b = g.getItemHeight(u.id) - 1;
        if (v || m) {
          var k = g.getItemPosition(u, c.start_date, c.end_date), f = document.createElement("div");
          return f.setAttribute(g.$config.item_attribute, u.id), f.className = ["gantt_resource_marker", v].join(" "), f.style.cssText = ["left:" + k.left + "px", "width:" + k.width + "px", "height:" + b + "px", "line-height:" + b + "px", "top:" + k.top + "px"].join(";"), m && (f.innerHTML = m), f;
        }
        return null;
      }
      function d(u, c) {
      ...
      return { render: function(u, c, h, _) {
        var g = c.$getTemplates(), v = c.getScale(), m = r(u, h.resource_property, c.getScale(), c), b = !!_, k = [];
        o[u.id] = {};
        ...
        if (k.length) {
          ...
          for (var w = 0; w < k.length; w++) x.appendChild(k[w]);
        }
        return x;
      }, update: function(u, c, h, _, g) {
        var v = h.$getTemplates(), m = h.getScale(), b = r(u, _.resource_property, h.getScale(), h), k = Ct(m, g), f = {};
        if (o && o[u.id]) for (var p in o[u.id]) f[p] = p;
        for (var y = k.start; y <= k.end; y++) {
          var $ = b[y];
          if (f[y] = !1, $) if (Wt(y, m, g, s)) if (o[u.id] && o[u.id][y]) o[u.id] && o[u.id][y] && !o[u.id][y].parentNode && c.appendChild(o[u.id][y]);
          else {
            var x = l(u, $, v, 0, h);
...
function Fs(t) {
  ...
  function o() {
    var _ = !!document.querySelector(".gantt_drag_marker"), g = !!document.querySelector(".gantt_drag_marker.gantt_grid_resize_area") || !!document.querySelector(".gantt_drag_marker.gantt_row_grid_resize_area"), v = !!document.querySelector(".gantt_link_direction"), m = t.getState(), b = m.autoscroll;
    return e = _ && !g && !v, !(!m.drag_mode && !_ || g) || b;
  }
  function l(_) {
  ...
  function c() {
    ...
    if (_) {
      ...
      var v = Y(_), m = s.x - v.x, b = s.y - v.y + window.scrollY, k = e ? 0 : h(m, v.width, a.x - v.x), f = h(b, v.height, a.y - v.y + window.scrollY), p = t.getScrollState(), y = p.y, $ = p.inner_height, x = p.height, w = p.x, T = p.inner_width, S = p.width;
      (f && !$ || f < 0 && !y || f > 0 && y + $ >= x + 2) && (f = 0), (k && !T || k < 0 && !w || k > 0 && w + T >= S) && (k = 0);
      var C = t.config.autoscroll_step;
      C && C < 2 && (C = 2), f *= C || 30, ((k *= C || 30) || f) && function(E, A) {
...
function Gs(t) {
  ...
  t._touch_events = function(s, r, o) {
    var l, d = 0, u = !1, c = !1, h = null, _ = null, g = null, v = null, m = [], b = null;
    let k = {};
    ...
    for (a.push([this.$container, "contextmenu", function(y) {
      ...
      if (k = y.touches.length, document && document.body && document.body.classList.add("gantt_touch_active"), !o(y)) if (y.touches && y.touches.length > 1) u = !1;
      else {
        h = r(y), b = function(x) {
              ...
              if (C) {
                ...
                for (let A = 0; A < T.length; A++) {
                  ...
                  if (I && I.childNodes && (M = I.querySelector(`[${t.config.task_attribute}="${S.id}"]`)), M) {
                    ...
                    M.parentNode.appendChild(D), t.$task_bars.appendChild(M), M.style.display = "none", m.push(M), M = null;
                  }
                }
              }
            }
          }(x), $._start_dnd(h), t._touch_drag = !0, t.refreshTask(x), t._touch_feedback())), g = null;
        }, t.config.touch_drag);
      }
      ...
      if (document && document.body && document.body.classList.remove("gantt_touch_active"), !o(y)) {
        ...
        var $ = r(y), x = i();
        if (x && x.on_mouse_up($), v && t.isTaskExists(v) && (t.refreshTask(v), m.length && (m.forEach(function(T) {
          T.parentNode && T.parentNode.removeChild(T);
        }), t._touch_feedback())), u = c = !1, m = [], v = null, h && d) {
          var w = /* @__PURE__ */ new Date();
          const T = k <= 1, S = _ && _.target.innerHTML == h.target.innerHTML;
  ...
  function i(a, s) {
    ...
    s && (r = t.getTaskByTime(), a.allow_root && r.unshift({ id: t.config.root_id, text: a.root_label || "" }), r = function(c, h, _) {
      ...
      for (var v = 0; v < c.length; v++) {
        var m = c[v];
        (m.id == _ || t.isChildOf(m.id, _) || g(m.id, m) === !1) && (c.splice(v, 1), v--);
      }
      return c;
...
function Ys(t) {
  ...
  return t.attachEvent("onAfterLightbox", function() {
    ...
    var h, _ = function(v, m) {
      return e.resources[m.id] || (e.resources[m.id] = v.querySelector(".gantt_resources")), e.resources[m.id];
    }(l, c), g = "";
    (function(v, m) {
      if (!e.filter[m.id]) {
        var b = v.querySelector(".gantt_resources_filter"), k = b.querySelector(".gantt_resources_filter_input"), f = b.querySelector(".switch_unsetted");
        e.filter[m.id] = { container: b, input: k, checkbox: f, filterApplied: !1 };
      }
      e.filter[m.id];
    })(l, c), function(v, m, b, k) {
      if (e.eventsInitialized[b.id]) return;
      var f = function($) {
        var x, w, T, S, C;
        y(b, v);
        var E = o(b);
        C = E.checkbox, S = E.input, T = C.checked, w = S.value.trim(), E.filterApplied = !!w, t.getState().lightbox && (m = t.getLightboxValues()), x = function(I, M, D, L) {
          var N, P;
          if (L) {
            var R = M[I.map_to] || [];
            ...
            N = function(W) {
              if (z[String(W.key)] && (D === "" || W.label.toLowerCase().indexOf(D.toLowerCase()) >= 0)) return W;
            };
          } else {
            if (D === "") return I;
            N = function(W) {
              if (W.label.toLowerCase().indexOf(D.toLowerCase()) >= 0) return W;
            };
          }
          return (P = t.copy(I)).options = wn(P.options, N), P;
        }(b, m, w, T);
        var A = m[b.map_to];
        k.form_blocks.resources.set_value(v, A, m, x);
      };
      function p($) {
        var x, w = $.target;
        if ($.target.type === "checkbox") {
          (x = w.parentNode.querySelector(s())).disabled = !w.checked;
      ...
      function y($, x) {
        var w = s(), T = x.querySelectorAll(w);
    ...
    }(l, u, c, this), ht(c.options, function(v, m) {
      c.unassigned_value != v.key && (h = function(b, k, f) {
        var p = {};
        if (k) {
          var y;
          Lt(k) ? y = Ee(k, function($) {
            return $.resource_id == f.key;
          }) : k.resource_id == f.key && (y = k), y && (p.value = y.value, p.id = y.id);
        }
    ...
    for (var c = r(u), h = [], _ = s(!0), g = s(!1), v = o(u), m = t.copy(e.resourcesValues[u.id]) || {}, b = c.querySelectorAll(_), k = c.querySelectorAll(g), f = 0; f < k.length; f++) delete m[k[f].getAttribute("data-item-id")];
    for (f = 0; f < b.length; f++) {
      var p = b[f].getAttribute("data-assignment-id"), y = b[f].getAttribute("data-item-id"), $ = b[f].value.trim();
      $ !== "" && $ !== "0" && (delete m[y], h[h.length] = { resource_id: y, value: $ }, p && (h[h.length - 1] = { ...h[h.length - 1], id: p }));
    }
    if (v.filterApplied) for (var x in m) h[h.length] = { resource_id: x, value: m[x].value, id: m[x].id };
    return h;
...
function Js(t) {
  ...
  return t.resource_table = { scale_height: 35, row_height: 35, columns: [{ name: "resource", label: "Resource", align: "center", width: 80, editor: i, template: function(o) {
    ...
    if (h.style.display = "none", t.callEvent("onSectionButton", [t._lightbox_id, d]) !== !1 && l.closest(".gantt_custom_button.gantt_add_resources")) {
      ...
      m && m.getItems().length == 0 && (g.style.display = "flex", v.style.display = "block"), function(b) {
        let k;
        const f = t.getTask(t._lightbox_id), p = t.getTaskAssignments(f.id);
        if (p.length) k = p[0].resource_id;
        else {
        ...
        y.addItem({ resource_id: k, task_id: f.id, duration: $.duration ?? t.calculateDuration(f), value: $.value, start_date: $.start_date ?? f.start_date, end_date: $.end_date ?? f.end_date, mode: $.mode }), t.refreshData();
      }(c), function(b) {
        const k = b.querySelectorAll(".gantt_row.gantt_row_task");
        if (k) {
          const f = k[k.length - 1].querySelector(".gantt_cell");
          if (f) {
    ...
    if (function(v) {
      ...
      (function(b, k, f, p) {
        if (t.$ui.getView("GridRL") && !e.gridID && t.$ui.getView("GridRL").destructor(), !e.gridID) {
          ...
          const $ = t.createDatastore({ name: "temp_resource_assignment_store", initItem: function(A) {
        ...
        t.$data.tempAssignmentsStore.attachEvent("onFilterItem", function(y, $) {
          return $.task_id == f.id && (!e.resource_filter_value || t.getDatastore(t.config.resource_store).getItem($.resource_id).text.toLowerCase().indexOf(e.resource_filter_value) > -1);
        }), t.refreshData();
      })(o, 0, d, u), function(b, k) {
        if (!e.filter[k.id]) {
          var f = b.querySelector(".gantt_resources_filter"), p = f.querySelector(".gantt_resources_filter_input");
          e.filter[k.id] = { container: f, input: p, filterApplied: !1 };
        }
        e.filter[k.id];
      }(o, u), function(b, k, f, p) {
        if (e.eventsInitialized[f.id]) return;
        var y = function(x) {
          var w, T, S;
          $(f, b);
          var C = r(f);
          S = C.input, T = S.value.trim(), e.resource_filter_value = T.toLowerCase(), C.filterApplied = !!T, t.getState().lightbox && (k = t.getLightboxValues()), w = function(A, I, M) {
            var D, L = t.copy(A);
            if (M === "") {
              L.resources = [];
            ...
            return D = function(N) {
              ...
              return P;
            }(I, A.options), L.resources = wn(L.resources, D), L;
          }(f, k, T);
          var E = k[f.map_to];
          p.form_blocks.resource_selector.set_value(b, E, k, w);
        };
        function $(x, w) {
          ...
          for (var C = 0; C < S.length; C++) {
            ...
            S[C].disabled ? delete e.resourcesValues[x.id][E] : e.resourcesValues[x.id][E] = { value: S[C].value, id: A };
          }
        }
        y = Sn(y, 100), r(f).container.addEventListener("keyup", y), r(f).container.addEventListener("input", y, !0), r(f).container.addEventListener("change", y, !0), e.eventsInitialized[f.id] = !0;
      }(o, l, u, this), h.style.display = "none";
      let v = document.querySelector(`.gantt_section_${u.name} .gantt_grid`), m = document.querySelector(`.gantt_section_${u.name} .gantt_resource_selector_filter_wrapper`);
      v.style.display = "none", m.style.display = "none";
    }
    const g = t.getDatastore("temp_resource_assignment_store");
    if (g) {
      let v = document.querySelector(`.gantt_section_${u.name} .gantt_resource_selector_filter_wrapper`), m = document.querySelector(`.gantt_section_${u.name} .gantt_grid`);
      g.getItems().length == 0 ? (h.style.display == "none" && (h.style.display = "flex"), v.style.display = "none", m.style.display = "none", _.style.display = "block") : (v.style.display = "flex", m.style.display = "block", _.style.display = "none");
    }
...
function Ks(t) {
  var n = function() {
    ...
    return F(p, f), p.prototype.render = function(y) {
    ...
    return F(y, p), y.prototype.render = function($) {
      ...
      f.form_blocks.textarea._get_input($).value = x || "";
    }, y.prototype.get_value = function($) {
      return f.form_blocks.textarea._get_input($).value;
    }, y.prototype.focus = function($) {
      var x = f.form_blocks.textarea._get_input($);
      f._focus(x, !0);
    ...
    return F(y, p), y.prototype.render = function($) {
      var x = f.form_blocks.getTimePicker.call(this, $);
      ...
      return T += x, $.single_date ? (x = f.form_blocks.getTimePicker.call(this, $, !0), T += "<span></span>") : T += "<span class='gantt_section_time_spacer'> &nbsp;&ndash;&nbsp; </span>", (T += x) + "</div>";
      ...
      f._focus($.getElementsByTagName("select")[0]);
    }, y;
    ...
    return F(y, p), y.prototype.render = function($) {
      ...
      f._focus($.querySelector("input[type=checkbox]"));
    }, y;
  }(t), r = function(f) {
    const p = ot();
    function y() {
    ...
    return F(y, p), y.prototype.render = function($) {
      ...
      f._focus($.querySelector("input[type=radio]"));
    }, y;
  }(t), o = function(f) {
    var p = ot();
    function y() {
    ...
    return F(y, p), y.prototype.render = function(T) {
      ...
      function z() {
        var W = x.call(f, T, E), G = w.call(f, T, E), bt = f.calculateEndDate({ start_date: W, duration: G, task: C }), Ht = f.templates.task_end_date || f.templates.task_date;
        O.innerHTML = Ht(bt);
      }
      function K(W) {
        var G = P.value;
    ...
    return F(y, p), y.prototype.render = function(w) {
      ...
      return $(A) || (I = f.form_blocks.getTimePickerValue(E, S)), { constraint_type: A, constraint_date: I };
    ...
    return F(y, p), y.prototype.render = function($) {
      var x = f.config.types, w = f.locale.labels, T = [], S = $.filter || function(A, I) {
    ...
    function x(S, C, E, A) {
      ...
      function qt(mt) {
        var X = Ot.value;
        X = $(A).parse(X), window.isNaN(X) && (X = 0), (X += mt) < 1 && (X = 1), Ot.value = $(A).format(X), $t();
      }
      z.querySelector(".baseline_delete_button").onclick = function(mt) {
        ...
        qt(1 * f.config.duration_step);
      }, f), bt[0].onchange = $t, bt[1].onchange = $t, bt[2].onchange = $t, bt[3] && (bt[3].onchange = $t), Ot.onkeydown = f.bind(function(mt) {
        var X;
        return (X = (mt = mt || window.event).charCode || mt.keyCode || mt.which) == f.constants.KEY_CODES.DOWN ? (qt(-1 * f.config.duration_step), !1) : X == f.constants.KEY_CODES.UP ? (qt(1 * f.config.duration_step), !1) : void window.setTimeout($t, 1);
      }, f), Ot.onchange = f.bind($t, f), f._resolve_default_mapping(A), K = C.start_date || /* @__PURE__ */ new Date(), W = C.end_date || f.calculateEndDate({ start_date: K, duration: 1, task: E }), G = f.calculateDuration({ start_date: K, end_date: W, task: E }), G = $(A).format(G), f.form_blocks._fill_lightbox_select(bt, 0, K, ri, A), Ot.value = G, $t();
    }
    function w(S, C) {
    ...
    return F(y, p), y.prototype.render = function(S) {
      ...
      if (f.callEvent("onSectionButton", [f._lightbox_id, E]) !== !1 && (C.closest(".gantt_custom_button.gantt_remove_baselines") && (A.innerHTML = f.locale.labels.baselines_section_placeholder), C.closest(".gantt_custom_button.gantt_add_baselines"))) {
        ...
        x(A, { id: f.uid(), task_id: I.id, start_date: I.start_date, end_date: I.end_date }, I, f._get_typed_lightbox_config()[S]);
      }
...
<div class="gantt_cal_ltitle_controls">
    ...
    for (var y = this._get_typed_lightbox_config(), $ = 0; $ < y.length; $++) if (y[$].map_to == f) {
    ...
    if (t.config.csp === !0 || t.env.isSalesforce ? t._lightbox_root = t.$root : t._lightbox_root = document.body, f === void 0 && (f = this.getLightboxType()), !this._lightbox || this.getLightboxType() != this.getTaskType(f)) {
      this._lightbox_type = this.getTaskType(f), p = document.createElement("div"), T = "gantt_cal_light", y = this._is_lightbox_timepicker(), t.config.wide_form && (T += " gantt_cal_light_wide"), y && (T += " gantt_cal_light_full"), p.className = T, p.style.visibility = "hidden", $ = this._lightbox_template, $ += "<div class='gantt_cal_lcontrols'>", $ += m(this.config.buttons_left), $ += "<div class='gantt_cal_lcontrols_push_right'></div>", $ += m(this.config.buttons_right), $ += "</div>", p.innerHTML = $, t._waiAria.lightboxAttr(p), t.config.drag_lightbox && (p.firstChild.onmousedown = t._ready_to_dnd, p.firstChild.addEventListener("touchstart", function(C) {
        t._ready_to_dnd(C.touches[0]);
      }), p.firstChild.onselectstart = function() {
        return !1;
      }, p.firstChild.style.cursor = "pointer", t._init_dnd_events()), this._lightbox && this.resetLightbox(), g(), this._cover.insertBefore(p, this._cover.firstChild), this._lightbox = p, x = this._get_typed_lightbox_config(f), $ = this._render_sections(x);
  ...
  function m(f, p) {
    var y, $, x = "";
    for ($ = 0; $ < f.length; $++) y = t.config._migrate_buttons[f[$]] ? t.config._migrate_buttons[f[$]] : f[$], x += "<div " + t._waiAria.lightboxButtonAttrString(y) + " class='gantt_btn_set gantt_left_btn_set " + y + "_set'><div dhx_button='1' data-dhx-button='1' class='" + y + "'></div><div>" + t.locale.labels[y] + "</div></div>";
    return x;
  }
  function b(f) {
    var p, y;
  ...
  function k(f, p, y) {
    ...
    switch (y.timeFormat[p]) {
      case "%Y":
        for (f._time_format_order[2] = p, f._time_format_order.size++, f.year_range && (isNaN(f.year_range) ? f.year_range.push && (w = f.year_range[0], T = f.year_range[1]) : $ = f.year_range), $ = $ || 10, x = x || Math.floor($ / 2), w = w || y.date.getFullYear() - x, T = T || t.getState().max_date.getFullYear() + x, S = w; S <= T; S++) E += "<option value='" + S + "'>" + S + "</option>";
        break;
      case "%m":
  ...
  t._init_lightbox_events = function() {
    t.lightbox_events = {}, t.lightbox_events.gantt_save_btn = function() {
      ...
      var p = f || window.event, y = f.target || f.srcElement, $ = it(y).indexOf("gantt_btn_set") > -1;
      switch ((f || p).keyCode) {
    ...
    for (let $ = 0; $ < y.length; $++) {
      ...
      if (!w) continue;
      let T = w.get_value.call(this, x, p, y[$], f), S = t._resolve_default_mapping(y[$]);
    ...
    var y = f, $ = p.getElementsByTagName("span"), x = [];
    ...
    for (f._time_format_order = { size: 0 }, t.config.limit_time_select && (S.first = 60 * T.first_hour, S.last = 60 * T.last_hour + 1, S.date.setHours(T.first_hour)), y = 0; y < S.timeFormat.length; y++) y > 0 && (w += " "), ($ = k(f, y, S)) && (x = t._waiAria.lightboxSelectAttrString(S.timeFormat[y]), w += "<select " + (f.readonly ? "disabled='disabled'" : "") + (p ? " style='display:none' " : "") + x + ">" + $ + "</select>");
    return w;
  }, getTimePickerValue: function(f, p, y) {
    var $, x = p._time_format_order, w = 0, T = 0, S = y || 0;
    return t.defined(x[3]) && ($ = parseInt(f[x[3] + S].value, 10), w = Math.floor($ / 60), T = $ % 60), new Date(f[x[2] + S].value, f[x[1] + S].value, f[x[0] + S].value, w, T);
  }, _fill_lightbox_select: function(f, p, y, $) {
    if (f[p + $[0]].value = y.getDate(), f[p + $[1]].value = y.getMonth(), f[p + $[2]].value = y.getFullYear(), t.defined($[3])) {
    ...
    return !1;
  }, t._delete_task_confirm = function({ task: f, message: p, title: y, callback: $, ok: x }) {
    t._simple_confirm(p, y, $, x);
  }, t._delete_link_confirm = function({ link: f, message: p, title: y, callback: $, ok: x }) {
    t._simple_confirm(p, y, $, x);
  }, t._simple_confirm = function(f, p, y, $) {
    if (!f) return y();
    var x = { text: f };
  ...
  if (!yt.isNode) {
    ...
    t.$ui = e.factory, t.$ui.layers = e.render, t.$mouseEvents = e.mouseEvents, t.$services.setService("mouseEvents", function() {
      ...
      return { getColumnIndex: function(c) {
        ...
        var _ = r(this), g = o(this), v = { position: 0 }, m = { position: 0 };
        _ && (m = _.getScrollState()), g && (v = g.getScrollState());
        var b = g && 1 * c == c, k = _ && 1 * h == h;
        if (b && k) for (var f = _._getLinkedViews(), p = g._getLinkedViews(), y = [], $ = 0; $ < f.length; $++) for (var x = 0; x < p.length; x++) f[$].$config.id && p[x].$config.id && f[$].$config.id === p[x].$config.id && y.push(f[$].$config.id);
        ...
        }).bind(this))), k && _.scroll(h);
        var w = { position: 0 }, T = { position: 0 };
        _ && (w = _.getScrollState()), g && (T = g.getScrollState()), this.callEvent("onGanttScroll", [v.position, m.position, T.position, w.position]);
      }, showDate: function(c) {
        ...
        var g, v = Math.max(_ - this.config.task_scroll_offset, 0), m = this._scroll_state().y;
        g = m ? h.top - (m - this.getTaskBarHeight(c)) / 2 : h.top, this.scrollTo(v, g);
        var b = a(this), k = i(this);
        b && k && b.$config.scrollY != k.$config.scrollY && l(this, b, "scrollY").scrollTo(null, g);
      ...
      for (var d in i._waiAria = { getAttributeString: function(u) {
        ...
        var h = i.config.links, _ = u.type == h.finish_to_start || u.type == h.start_to_start, g = u.type == h.start_to_start || u.type == h.start_to_finish, v = i.locale.labels.link + " " + i.templates.drag_link(u.source, g, u.target, _);
